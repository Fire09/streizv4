...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
   -- dwRefreshChipCount = false
    dwCurrentChipCount = RPC.execute("rd-casino:getCurrentChipCount")
  end
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['rd-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                        ...
                        -- exports["rd-ui"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["rd-ui"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['rd-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['rd-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
...
RegisterUICallback("rd-ui:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("rd-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("rd-casino:winChips", currentBetAmount / 2)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "rd-ui:blackjackHitStand",
...
RegisterUICallback("rd-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("rd-ui:application-closed", function(name, data)
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("rd-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("rd-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
