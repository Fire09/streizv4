...
Citizen.CreateThread(function()
  for _, v in pairs(LAUNDRY_COORDS) do
    exports["rd-polytarget"]:AddBoxZone("casino_hotel_laundry_machine", v[1], v[2], v[3], v[4])
  end
  exports["rd-interact"]:AddPeekEntryByPolyTarget("casino_hotel_laundry_machine", {{
    id = "casino_laundry_interact",
    label = "Do Laundry",
    icon = "money-bill-alt",
    event = "rd-casino:hotel:doLaundry",
  }}, { distance = { radius = 1.5 } })
end)

AddEventHandler("rd-casino:hotel:doLaundry", function(p1, p2, pContext)
  local laundryId = pContext.zones["casino_hotel_laundry_machine"].id
  local coins = exports["rd-inventory"]:getItemsOfType("casinogoldcoin", 1, true)
  local coin = coins[1]
  if not coin then
  ...
  local canLaunder = RPC.execute("rd-casino:hotel:canUseLaundry", laundryId, roomId, true)
  if not canLaunder then
    return false
  end
  
  TriggerEvent("animation:PlayAnimation", "cokecut")
  local finished = exports["rd-taskbar"]:taskBar(5000, [[Adding "Laundry"...]])
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end
  RPC.execute("rd-casino:hotel:finishup", laundryId , {} )
end)
