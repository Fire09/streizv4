...
local function drawStatusHUD(state, pValues)
  ...
  local balance = RPC.execute("rd-casino:getCurrentChipCount")
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("rd-slotmachines-sv:checkIfSlotMachinesState")
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("rd-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "rd-ui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "rd-ui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("rd-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["rd-ui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["rd-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            elseif exports["rd-inventory"]:hasEnoughOfItem("casinovip", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            else
            ...
            exports["rd-ui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end

        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("rd-slotmachines-sv:sitDownAtSlotMachine", k)
            break
          end
...
--#[Event Handlers]#--
RegisterNetEvent("rd-slotmachines-cl:getMachineData")
AddEventHandler("rd-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("rd-slotmachines-cl:updateMachineData")
AddEventHandler("rd-slotmachines-cl:updateMachineData", function(machine, ply)
  if currentSlotMachineObjs[machine] ~= nil then
    currentSlotMachineObjs[machine].ply = ply
  end
end)

RegisterNetEvent("rd-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("rd-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("rd-slotmachines-cl:playSlotMachineSound")
AddEventHandler("rd-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("rd-slotmachines-cl:stopReels")
AddEventHandler("rd-slotmachines-cl:stopReels", function(reel,rot)
...
RegisterNetEvent("rd-slotmachines-cl:spinFinished")
AddEventHandler("rd-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    -- RPC.execute("rd-casino:winChips", amountWon)
...
RegisterNetEvent("rd-slotmachines-cl:toggleSlotMachines")
AddEventHandler("rd-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("rd-casino:casinoEnteredEvent", function()
  isInsideCasino = true
  isSlotMachinesEnabled, currentAccJackpot = RPC.execute("rd-slotmachines-sv:checkIfSlotMachinesState")
...
AddEventHandler("rd-casino:casinoExitedEvent", function()
  isInsideCasino = false
  isSlotMachinesEnabled = RPC.execute("rd-slotmachines-sv:checkIfSlotMachinesState")
  if not isSlotMachinesEnabled then
    return
  end
...
--#[NUI Callbacks]#--
RegisterUICallback("rd-ui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    local useChipsResult = RPC.execute("rd-casino:useChips", currentBet)
    ...
    TriggerServerEvent(
      "rd-slotmachines-sv:spinMachine",
...
RegisterUICallback("rd-ui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
