...
RegisterUICallback('heli:setOverlayText', function (data, cb)
    ...
    exports["rd-hud"]:sendAppEvent("newscam", { text = CurrentOverlayText })
end)

RegisterUICallback('heli:toggleOverlay', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	CurrentOverlay = not CurrentOverlay
	exports["rd-hud"]:sendAppEvent("newscam", { show = CurrentOverlay })
    exports["rd-hud"]:sendAppEvent("hud", { display = not CurrentOverlay })
end)
...
Citizen.CreateThread(function()
  exports["rd-keybinds"]:registerKeyMapping("","Heli", "Cam", "+heliCam", "-heliCam")
  RegisterCommand('+heliCam', heliCam, false)
  RegisterCommand('-heliCam', function() end, false)
  
  exports["rd-keybinds"]:registerKeyMapping("","Heli", "Vision", "+heliVision", "-heliVision")
  RegisterCommand('+heliVision', heliVision, false)
  RegisterCommand('-heliVision', function() end, false)
  
  exports["rd-keybinds"]:registerKeyMapping("","Heli", "Rappel", "+heliRappel", "-heliRappel")
  RegisterCommand('+heliRappel', heliRappel, false)
  RegisterCommand('-heliRappel', function() end, false)
  
  exports["rd-keybinds"]:registerKeyMapping("","Heli", "Spotlight", "+heliSpot", "-heliSpot")
  RegisterCommand('+heliSpot', heliSpot, false)
  RegisterCommand('-heliSpot', function() end, false)
  
  exports["rd-keybinds"]:registerKeyMapping("","Heli", "Lock On", "+heliLock", "-heliLock")
...
AddEventHandler("rd-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if helicam then
			...
			helicam = false
			exports["rd-ui"]:sendAppEvent("newscam", { show = false })
			exports["rd-ui"]:sendAppEvent("hud", { display = true })
			ClearTimecycleModifier()
