...
RegisterNetEvent('rd-base:PolyZoneUpdate')
AddEventHandler('rd-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "rd-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('rd-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                -- print('rd-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('rd-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                -- print('rd-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('rd-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                -- print('rd-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('rd-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                -- print('rd-polyzone:stop')
            end
        end
