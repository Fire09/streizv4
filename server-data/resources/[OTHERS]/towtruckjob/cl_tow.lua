...
AddEventHandler('jobs:towVehicle', function (pParams, pEntity, pContext)
  local target = exports['rd-target']:GetEntityInFrontOfEntity(pEntity, -8, 0.6)

  if target == nil or target == 0 then 
    return TriggerEvent("DoLongHudText","Araç bulunamadı", 2)
  end
  ...
  if (BlacklistedModels[targetModel] or BlacklistedClasses[targetClass]) then
    return TriggerEvent("DoLongHudText","Bu araç çekilemez", 2)
  end

  local targetDriver = GetPedInVehicleSeat(target, -1)

  if targetDriver ~= 0 then return TriggerEvent("DoLongHudText","Araç boş olmalıdır.", 2) end
  ...
  if target and pEntity ~= target and GetEntityType(target) == 2 and GetEntityModel(pEntity) == `flatbed` then
    ...
    if distance <= 10 then
      ...
      local hooking = exports["rd-taskbar"]:taskBar(15000,"Araç bağlama.")
      if hooking == 100 then
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        Citizen.Wait(1)
        local towing = exports["rd-taskbar"]:taskBar(5000,"Araç Çekiliyor")
  ...
  TriggerEvent("DoLongHudText","Araç bulunamadı", 2)
end)
...
AddEventHandler('jobs:untowVehicle', function (pParams, pEntity, pContext)
  ...
  if target and target ~= pEntity and target ~= 0 then
    ...
    Citizen.Wait(1000)
    TriggerEvent("animation:tow")
    local untowing = exports["rd-taskbar"]:taskBar(7000,"Araç Çekme")

    if untowing == 100 then
      TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
      Citizen.Wait(1)
      local unhooking = exports["rd-taskbar"]:taskBar(5000,"Araç Kancasını Açılıyor")
