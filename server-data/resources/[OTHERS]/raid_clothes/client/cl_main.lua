...
RegisterNetEvent("rd-clothes:notsave")
AddEventHandler("rd-clothes:notsave", function()
    TriggerEvent("rd-spawn:finishedClothing","Old")
    LoadPed(oldPed)
end)

RegisterNetEvent("rd-clothing:save")
AddEventHandler("rd-clothing:save", function()
...
function Save(save, close)
    ...
    TriggerEvent('rd-weapons:getAmmo')
...
RegisterUICallback("rd-ui:raid_clothes:addOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    Wait(1) --wait to fix ui bug?
    exports['rd-ui']:openApplication('textbox', {
        callbackUrl = 'rd-ui:raid_clothes:addOutfit',
        key = 1,
        items = {
            {
                icon = "pencil-alt",
...
RegisterUICallback("rd-ui:raid_clothes:addOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterUICallback("rd-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports["rd-ui"]:hideContextMenu()
end)

RegisterUICallback("rd-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
    ...
    for i = 1, #skincheck do
        ...
        menuData[#menuData + 1] = {
            ...
            icon = 'tshirt',
            children = {
                { title = "Change Outfit", action = "rd-ui:raid_clothes:changeOutfit", icon = 'edit', key = slot},
                { title = "Delete Outfit", action = "rd-ui:raid_clothes:deleteOutfit", icon = 'trash', key = slot},
            }
        }
    end
    if #menuData >= 0 then
        if #menuData < 20 then
            ...
            menuData[#menuData + 1] = {
                ...
                description = '',
                key = emptySlot,
                action = "rd-ui:raid_clothes:addOutfitPrompt",
                icon = 'save'
            }
        end
...
function SetCustomNuiFocus(hasKeyboard, hasMouse)
  ...
  -- TriggerEvent("rd-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse)
end
...
AddEventHandler("rd-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        currentShop = zone
        exports["rd-ui"]:showInteraction(("[M] %s"):format(currentZone.text))
...
function listenForKeypress(zoneName, zoneData, isFree)
    listening = true
    Citizen.CreateThread(function()
...
RegisterNetEvent("rd-context:closeglobal")
AddEventHandler("rd-context:closeglobal", function()
    ...
    local data = {
        {
            ...
            description = "",
            key = true,
            action = "rd-ui:raid_clothes:CustomaddOutfitPrompt"
        },
    }
...
RegisterUICallback("rd-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['rd-ui']:openApplication('textbox', {
        callbackUrl = 'rd-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterUICallback("rd-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
