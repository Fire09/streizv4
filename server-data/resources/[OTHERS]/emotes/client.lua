...
AddEventHandler("Animation:Set:Gait", function(pArgs)
    ...
    if setGait == "move_m@swagger" then
      if exports["rd-inventory"]:hasEnoughOfItem("pimpcane", 1) then
        TriggerEvent("rd-propattach:attach", "prop_cs_walking_stick")
      end
    end
end)
...
anims = {
    ...
    ["c"] = function(ped)
        if exports["rd-flags"]:HasPedFlag(PlayerPedId(), 'isSittingOnChair') then
          TriggerEvent("rd-emotes:sitOnChair")
        elseif exports["rd-flags"]:HasPedFlag(PlayerPedId(), 'isPoledancing') then
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["rd-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["rd-taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["smoke"] = function(ped)
        ...
        if exports["rd-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["rd-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                SetCurrentPedWeapon(ped, 0xA2719263, true)
...
animsDog = {
    ...
    ["carenter"] = function(ped)
        local animDict = "creatures@rottweiler@in_vehicle@std_car"
        local currentTarget = exports['rd-target']:GetCurrentEntity()
        ...
        local currentTargetCoords = exports['rd-target']:GetCurrentTargetCoords()
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1744.53, 2646.94, 45.61)) <
            ...
            10.0) then
        local finished =
            exports["rd-taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
