...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(player) then
            ...
            local isFromBeatdown = exports["rd-police"]:getIsInBeatmode()

            if isFromBeatdown then
                exports["rd-police"]:setIsInBeatmode(false)
                exports["rd-police"]:setBeatmodeDebuff(true)
            end

            if imDead == 0 then
                imDead = 1
                TriggerEvent('rd-hospital:client:RemoveBleed')
                deathTimer()
            end
        end
    end
...
RegisterNetEvent('close:mouse')
AddEventHandler('close:mouse', function()
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["rd-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    TriggerServerEvent('rd-healthcare:playerRespawn', hosp)
...
RegisterNetEvent('ragdoll:alert')
AddEventHandler('ragdoll:alert', function()
