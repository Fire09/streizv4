...
RegisterNetEvent("rd-inventory:itemCheck", function(item, hasItem, quantity)
    ...
    if not hasItem then
        if item == "boxinggloves" then
            ...
            TriggerEvent("rd-inventory:boxingGlovesEquipped", false)
        end
        SetDefaultValues()
        return
    end
    ...
    for buff, value in pairs(buffs) do
        ...
        if buff == "meleedamage" then
            ...
            if item == "boxinggloves" then
                ...
                TriggerEvent("rd-inventory:boxingGlovesEquipped", true)
...
Citizen.CreateThread(function()
    ...
    local buffsEnabled = exports["rd-config"]:GetMiscConfig("buffs.enabled")
