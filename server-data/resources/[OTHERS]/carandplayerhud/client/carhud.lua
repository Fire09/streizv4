...
AddEventHandler("roll", function(times, weight)
  ...
  local randomweight = math.random(weight)
  rollAnim()
  local strg = ""
  for i = 1, times do
    if i == 1 then
...
toggleFaceWear = function(pWearType, pShouldRemove)
  ...
  if pWearType == 1 and not pShouldRemove and removedHatManually then
    local hasHat = exports["rd-inventory"]:hasEnoughOfItem("hat", 1, false, true, {hat = facialWear[PropIndex+1]["Prop"]})
    if not hasHat then
  ...
  if pWearType == 4 and not pShouldRemove and removedMaskManually then
    local hasMask = exports["rd-inventory"]:hasEnoughOfItem("mask", 1, false, true, {mask = facialWear[pWearType]["Prop"]})
    if not hasMask then
    ...
    if pWearType ~= 5 then
      ...
      if pWearType == 4 then
        ...
        if currentDrawable ~= -1 then
          ...
          local hasPropItem = exports["rd-inventory"]:hasEnoughOfItem("mask", 1, false, true, itemMeta)
        ...
        if pWearType ~= 2 then
          ...
          if pWearType == 1 and currentProp ~= -1 then
            ...
            local hasPropItem = exports["rd-inventory"]:hasEnoughOfItem("hat", 1, false, true, itemMeta)
            if not hasPropItem then
...
AddEventHandler("rd-jobmanager:playerBecameJob", function(job, name)
  ...
  if DoesEntityExist(objFound) then
    -- TriggerEvent("DoLongHudText","Press [E] to leave the bed at any time.",1)
      ...
      if not disableKeys then
        showInteraction = true
        exports["rd-ui"]:showInteraction("[E] Leave Bed; [F] Dismiss")
      end
...
Citizen.CreateThread(function()
  while true do
    local isSitting = exports["rd-flags"]:HasPedFlag(PlayerPedId(), "isSittingOnChair")
...
RegisterNetEvent('rd-voice:focus:set')
AddEventHandler('rd-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
RegisterNetEvent('rd:admin:maxstats')
AddEventHandler('rd:admin:maxstats', function()
    currentValues["thirst"] = 100
    currentValues["hunger"] = 100 
    TriggerEvent("heal", PlayerPedId())
    TriggerEvent("Hospital:HealInjuries", PlayerPedId(),true) 
    TriggerServerEvent("rd-death:reviveSV", source)
...
AddEventHandler('changehunger', function(pNewValue)
  local value = (pNewValue and pNewValue > 0) and pNewValue or 10
  currentValues["hunger"] = currentValues["hunger"] + value
...
function initHud(hasStarted)
  ...
  if hasStarted then
    ...
    if initialHudSettings ~= "{}" then
      exports["rd-hud"]:sendAppEvent("preferences", initialHudSettings)
      exports["rd-ui"]:sendAppEvent('preferences', initialHudSettings)
      TriggerEvent("rd-preferences:setPreferences", initialHudSettings)
    end
    ...
    if counter == 0 then
     ...
     -- exports["rd-hud"]:sendAppEvent("hud", {
      ...
      if valueChanged then
        -- RPC.execute("rd-hud:UpdateNeeds", currentValues)
        exports["rd-hud"]:startHealthArmorUpdates()
        exports["rd-hud"]:sendAppEvent("hud", {
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("rd-buffs:addUIBuff", { buffedHunger = true, buffedThirst = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 6hr
    buffStartTime = 0
    TriggerEvent("rd-buffs:addUIBuff", { buffedHunger = false, buffedThirst = false })
  end)
end)
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("rd-buffs:addUIBuff", { buffedHealth = true, buffedArmor = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 3hr
    citrineStartTime = 0
    TriggerEvent("rd-buffs:addUIBuff", { buffedHealth = false, buffedArmor = false })
  end)
end)
Citizen.CreateThread(function()
  while true do
...
AddEventHandler("rd-inventory:itemUsed", function(item)
  ...
  local finished = exports["rd-taskbar"]:taskBar(4000, "Using Chapstick")
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  local hasLicense = exports["rd-gov"]:HasLicensePilot()
  if hasLicense then
    return
  end
