...
async function rpcExecute(name, ...params) {
    ...
    // TriggerNetClientEvent("striez:rpc:request", params[0], Resource, name, callID, params, false)
    emitNet("striez:rpc:request", params[0], Resource, name, callID, params, false)
...
RegisterServerEvent("striez:rpc:request")
on('striez:rpc:request', async (origin, name, callID, params, boolean) => {
    const src = source
    if (boolean == true) {
        return
    }
    ...
    // TriggerNetClientEvent("striez:rpc:response", src, origin, callID, response, boolean);
    emitNet("striez:rpc:response", src, origin, callID, response, boolean);
})

RegisterServerEvent("striez:rpc:response")
on('striez:rpc:response', async (pOrigin, pCallId, pResponse, pPacked) => {
