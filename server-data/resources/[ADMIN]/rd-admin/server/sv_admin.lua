...
function getPrioType(pSource)
	local pSrc = pSource
	local user = exports['rd-base']:getModule("Player"):GetUser(pSource)
	return "Normal :) ~ Riddle Community"
end
...
function getCurrentPlayers()
    ...
    for idx, player in ipairs(players) do
        ...
        local user = exports["rd-base"]:getModule("Player"):GetUser(player)
        if user then 
...
function runCommandFromClient(pSource, pAction, pData)
    ...
    if command.isClientCommand then
        TriggerClientEvent("rd-admin:runClientCommand", pSource, pAction, pSource, pData, command.log)
    else
...
RegisterServerEvent("rd-admin:shorseon")
AddEventHandler("rd-admin:shorseon", function()
    pOn = true
    TriggerClientEvent("rd-admin:horse", -1, pOn)
end)

RegisterServerEvent("rd-admin:shorseoff")
AddEventHandler("rd-admin:shorseoff", function()
    pOn = false
    TriggerClientEvent("rd-admin:horse", -1, pOn)
end)