...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["rd-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["rd-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    if(data.toSelect){
        global.exports["rd-admin"].enterSelection();
    }
...
on("onResourceStart", (resourceName) => {
    if(GetCurrentResourceName() != 'rd-admin') {
      return;
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    if(data.inside){
        ...
        emit('rd-binds:should-execute',false)
        ...
        emit('rd-binds:should-execute',true)
        if(blockingActions){
...
global.exports["rd-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
