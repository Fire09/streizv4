...
RegisterNetEvent('rd-weapons:destroyGroundWeapon')
AddEventHandler('rd-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('rd-weapons:allowDrop')
AddEventHandler('rd-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('rd-weapons:dropweapon')
AddEventHandler('rd-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("rd-weapons:dropWeapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('rd-weapons:doHandOver')
AddEventHandler('rd-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('rd-weapons:dropWeaponSearch')
AddEventHandler('rd-weapons:dropWeaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('rd-weapons:giveweapon')
AddEventHandler('rd-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("rd-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["rd-inventory"]:GetCurrentWeapons() 
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())
...
local ScopedWeapons = {
	  ...
	  for weaponHash, v in pairs(ScopedWeapons) do
		if GetSelectedPedWeapon(PlayerPedId()) == weaponHash and IsPlayerFreeAiming(PlayerId()) then
		  if not isScoped then
			isScoped = true
			exports["rd-ui"]:sendAppEvent("sniper-scope", { show = true })
		  end
		elseif isScoped and not IsPlayerFreeAiming(PlayerId()) then
		  isScoped = false
		  exports["rd-ui"]:sendAppEvent("sniper-scope", { show = false })
		end
	  end
	  Citizen.Wait(100)
	end
  ...
  CreateThread(function()
	...
	while true do
	  ...
	  if RecentlyRiding then
		local BikeArmor = exports["rd-inventory"]:hasEnoughOfItem("bikearmor", 1, false,true)
		if BikeArmor then
  ...
  function ProcessHitByCash(pAttacker)
	TriggerServerEvent("rd-weapons:attackedByCash", pAttacker)
  end
  ...
  AddEventHandler("DamageEvents:EntityDamaged", function(victim, attacker, pWeapon, isMelee)
	...
	-- print(pWeapon)
	if ( (pWeapon == CrashHash or pWeapon == RamHash or pWeapon == FallHash) and exports["rd-inventory"]:hasEnoughOfItem("bikearmor", 1, false, true) and not DegApplied and RecentlyRiding) then
	  DegApplied = true
...
Citizen.CreateThread(function()
    ...
    -- Register Keybinds
    exports["rd-keybinds"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("rd-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
...
RegisterNetEvent("rd-weapons:client:weaponEquiped")
AddEventHandler("rd-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("rd-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("rd-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("rd-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
