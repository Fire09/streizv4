...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://rd-inventory/UpdateSettings',
            JSON.stringify({
...
function UpdateQuality(data, penis) {
    ...
    let htmlstring = DOMPurify.sanitize(
        ...
        "<span class='item-qty'>" + itemcount + "</span> <span class='item-weight'>" + weight.toFixed(1) + "</span> </div>"
        +
...
function invStack(
    ...
    $.post('https://rd-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://rd-inventory/move', JSON.stringify(arr));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0, targetInvSlots = 40) {
    ...
    if (targetinvName.indexOf('Drop') > -1) {
        ...
        secondaryMaxWeight = 250.0;
        slotLimitTarget = 40;
        const stateId = targetinvName.split("-")[1];
        displayName = `Other Player (${stateId})`;
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://rd-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
            ...
            if (itemList[itemid] !== undefined) {
                ...
                if ((TargetInventoryName == 'Shop' || isPlayerShop) && !main) {
                    ...
                    htmlstring = DOMPurify.sanitize(
                        ...
                        "<span class='item-qty'>" + (isPlayerShop ? itemcount  : !stackable ? '1' : '') + " </span> <span class='item-weight'>" + weight.toFixed(1) + "</span> </div>"
                ...
                if (TargetInventoryName.indexOf('Craft') > -1 && !main) {
                    ...
                    let fill = DOMPurify.sanitize(
                        ...
                        name +
                        " </div> <div class='information'> " +
                        "<span class='item-qty'>" + itemcount + "</span> <span class='item-weight'>" + weight.toFixed(1) + "</span>"
                        + `</div> <img src='${DOMPurify.sanitize(image)}` +
                        "' data-inventory='" +
                        inventoryNumber +
                        "' data-creationDate='" +
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://rd-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function FindNextSlotAndMove(half) {
    ...
    for (let i = startingSlot; i <= slotCounter; i++) {
...
function UpdateTextInformation(slot) {
    if (document.getElementById(slot)) {
        ...
        if (item) {
            ...
            informationDiv.html("<span class='item-qty'>" + amount + "</span> <span class='item-weight'>" + weight.toFixed(1) + "</span>");
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://rd-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://rd-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://rd-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://rd-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://rd-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://rd-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://rd-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://rd-inventory/craftProgression', JSON.stringify({
            ...
            if (itemList[itemid].weapon) {
              $.post('https://rd-inventory/weaponCrafted', JSON.stringify({
                "item": itemid,
              }));
            }
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://rd-inventory/invuse', JSON.stringify(arr));
...
function setPriceForItem(item,targetName, itemData) {
    if(priceEdit == ''){
        ...
        if(price >= 1){
            ...
            $.post('https://rd-inventory/invSetPrice', JSON.stringify(information));
