CreateThread(function()
  exports["rd-polyzone"]:AddBoxZone("ottos_auto", vector3(820.39, -805.03, 26.18), 59.0, 59.8, {
    minZ=22.98,
    maxZ=31.78,
    heading=0
  })
...
AddEventHandler("rd-ottosauto:vehicle:transaction", function(pArgs, pEntity, pEntityFlags, pEntityCoords)
  ...
  if action and (action == 'sell' or action =='buy') then
    exports['rd-ui']:openApplication('textbox', {
        callbackUrl = 'rd-ui:ottosauto:transaction',
        ...
        hiddenItems = {
          vehicleVIN = exports['rd-vehicles']:GetVehicleIdentifier(pEntity),
...
RegisterUICallback("rd-ui:ottosauto:transaction", function(data, cb)
  ...
  STX.Procedures.execute("rd-ottosauto:vehicle:transaction", action, workerId, customerId, price, vehicleVIN, businessCode, vehicleEntityHandle, workerServerId)
end)

RegisterUICallback('rd-ottosauto:business:sale:accepted', function(data, cb)
  ...
  local success, errMessage = STX.Procedures.execute("rd-ottosauto:vehicle:accepted", workerId, customerId, businessAccountId, price, transactionLog:format(vehiclePlate, workerId, sellerFirstName, sellerLastName), transactionType, vin, vehicleEntityHandle, workerServerId, businessCode)
