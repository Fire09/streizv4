...
function setCoords(x, y) {
    $.post('https://rd-dispatch/set-coords', JSON.stringify({
        x: x,
        y: y
    }));
}
...
$(document).ready(function() {
	window.addEventListener('message', function(event){
		if (event.data.action == "open") {
            ...
            $('.mesai-nopixel-yok').html(createTablePolice(table));
        ...
        if (event.data.action == "update-html") {
            ...
            if (event.data.code) {
                ...
                $('.mesai-nopixel-yok').append(event.data.code);
        ...
        if (event.data.action == "close") {
            $.post('https://rd-dispatch/close', JSON.stringify({}));
            outKapa();
		}

        if (event.data.action == "close-callsign") {

            $.post('https://rd-dispatch/close-callsign', JSON.stringify({}));
            outKapa();
		}
        ...
        if (event.data.action == "deletenopixel") {
        ...
        $('.create-call').click(function() {
            $.post('https://rd-dispatch/create_call', JSON.stringify({
                number: npnopixelNumber
            }));
        })
        ...
        $('.dismiss-pingg').click(function() {
            createCall3.addClass('dismissed');
            $('.called-list').fadeOut();
            $.post('https://rd-dispatch/dismiss_call-2', JSON.stringify({
                number: npnopixelNumber
            }));
        })
        ...
        if (event.data.action == "add_police-assign") { 
            var esekcure = event.data.nopixel;
            asignosurukcureniyecalismionopixelinisikiyimyeterhayatimisikiyimya(esekcure);
        }
        ...
        //     $('.mesai-nopixel-yok').append(policeCreat2(event.data.code, event.data.vehicle));
    ...
    // $('.mesai-nopixel-yok').append(createTablePolice(policeTable.policeTables))
    ...
    function vehiclesClick() {
        $('.vehicles').click(function() {
            ...
            if (value == 1) {
                ...
                $('.mesai-nopixel-yok').append(policeCreat2(policeName, icon));
                removeItem()
                mesaiHold()
                $.post('https://rd-dispatch/change-vehicle', JSON.stringify({
                    name: policeName,
                    icon: icon
                }));
                ...
                $.post('https://rd-dispatch/table_ayril', JSON.stringify({
                    name: remove
                }));
    ...
    $('.dismiss-ping').click(function() {
        ...
        $.post('https://rd-dispatch/dismiss_call-1', JSON.stringify({
            number: npnopixelNumber
        }));
    })
    ...
    function assing() {
        $('.notify-border').on('mousedown', function(e) {
            npnopixelNumber = Number($.trim($(this).children('div').first().children('div').first().children('div').first().children('span').text()))
    ...
    function assignButtons(){
        $('.assing-buttons').click(function() {
            $.post('https://rd-dispatch/add_police-assign', JSON.stringify({
                npnopixel: '.assing-buttons'
            }));
        })
    }

    function asignosurukcureniyecalismionopixelinisikiyimyeterhayatimisikiyimya(sa) {
    ...
    $('.crate-dismiss').click(function() {
        ...
        $.post('https://rd-dispatch/create_call-2', JSON.stringify({
            number: npnopixelNumber
        }));
    })
    ...
    $('.remove-dismis-ping').click(function() {
        $('.dismiss-list').fadeOut();
        createCall4.remove();
        $.post('https://rd-dispatch/remove-call', JSON.stringify({
            number: npnopixelNumber
        }));
    })
    ...
    function cureungotunu() {
        ...
        $('.remove-unit').click(function() {
            ...
            $.post('https://rd-dispatch/delete', JSON.stringify({
                name: player
            }));
            ...
            $('.mesai-personel').each(function() {
                if ($.trim($(this).text()) == text) {
                    ...
                    if (owner == player) {
                        ...
                        $.post('https://rd-dispatch/under-police', JSON.stringify({
                            player: player,
                            owner: owner
                        }));
                    }
    ...
    function callCameRemove(){
        $('.call-came').dblclick(function(e){
            ...
            $('.remove-list .MuiPaper-rounded ul li').click(function(){
                $.post('https://rd-dispatch/delete_police-assign', JSON.stringify({
                    npnopixel: callCame
                }));
            })
        })
    ...
    // Save
    function save(){
        var save = $('.mesai-nopixel-yok').html()
        
        $.post('https://rd-dispatch/save', JSON.stringify({
            html: save
        }));
