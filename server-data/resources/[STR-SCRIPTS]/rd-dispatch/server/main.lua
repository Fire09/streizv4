...
AddEventHandler("dispatch:add-police", function(code, name, icon, under, coords)
    ...
    print("Add Some One With Code: " .. code .. " name: " .. name .. " icon: " .. icon)
end)
...
AddEventHandler("dispatch:add_police-assign", function(nopixelsizkurt)
    TriggerClientEvent("dispatch:add_police-assign_cl", -1, nopixelsizkurt)
end)
...
AddEventHandler("dispatch:html", function(nopixel)
    local src = source
    html = nopixel
...
RegisterNetEvent("dispatch:pixel-html")
AddEventHandler("dispatch:pixel-html", function()
    local src = source
    TriggerClientEvent("dispatch:get-html", src, html)
end)
...
RegisterCommand('callsign', function(source, args)
    ...
    exports.oxmysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character.id}, function(result)
        if result[1].job == 'police' or result[1].job == 'ems' or result[1].job == 'state' or result[1].job == 'ranger' or result[1].job == 'sheriff' then
            exports.oxmysql:execute("UPDATE jobs_whitelist SET `callsign` = @callsign WHERE cid = @cid", {
                ['callsign'] = args[1],
                ['cid'] = character.id
            })
            TriggerClientEvent('police:setCallSign', src, args[1], args[2])
        else
