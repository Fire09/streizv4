...
AddEventHandler('bank:openbank', function()
  ...
  open = not open
  bankanimation()
  local finished = exports["rd-taskbar"]:taskBar(1500,"Opening Banking")
...
RegisterNUICallback('bankWithdraw', function(data, cb)
  ...
  if success then
    --print("success")
    TriggerServerEvent('rd-banking:withdraw_log', data.amount, data.comment)
    local accounts, cash, name = RPC.execute("GetAccounts")
    local transactions = RPC.execute("GetTransactionLogs", data.id)
    SendReactMessage('bankData', {
      type = banktype,
...
RegisterNUICallback('bankDeposit', function(data, cb)
  ...
  if success then
    --print("success")
    TriggerServerEvent('rd-banking:deposit_log', data.amount, data.comment)
    local accounts, cash, name = RPC.execute("GetAccounts")
    local transactions = RPC.execute("GetTransactionLogs", data.id)
    SendReactMessage('bankData', {
      type = banktype,
