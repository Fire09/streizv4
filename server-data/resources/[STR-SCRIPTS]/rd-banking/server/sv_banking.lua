...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("rd-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    ...
    if id == "personal" then
        ...
        local userSender = exports["rd-base"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["rd-base"]:getModule("Player"):GetUser(target)
  ...
  RegisterCommand('givecash', function(source, args)
    ...
    if tonumber(args[1]) == src or tonumber(args[2]) <= 0 then
      ...
      if tonumber(args[2]) then
        ...
        PerformHttpRequest('https://discord.com/api/webhooks/972270759412133913/JSOcZmDJWqj6q0JJ2yTcLBdsYPaeASvEGnbuXz6yiJSh_WYGQdSFKctkU6loG8K4LJYe', function(err, text, headers) end, 'POST', json.encode({username = "np",  avatar_url = "https://cdn.discordapp.com/attachments/928946993840132116/930453062403899472/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
       end
    end
  end)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["rd-base"]:getModule("Player"):GetUser(src)
  local player = exports["rd-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('rd-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('rd-banking:addMoney')
AddEventHandler('rd-banking:addMoney', function(amount)
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('rd-banking:removeMoney')
AddEventHandler('rd-banking:removeMoney', function(amount)
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("rd-banking:business_money", function(amt, account, case)
...
local Deposit_Log = "https://discord.com/api/webhooks/960182729180348416/Q2z-vluHnu2IsoKHZ2VB-BE2ORpABZJ2f5KEUBZI0uFFPbUxYymLqU_j8mH523dZoZ_s"

RegisterServerEvent('rd-banking:deposit_log')
AddEventHandler('rd-banking:deposit_log', function(pDepositAmt, pDepositComment)
    ...
    PerformHttpRequest(Deposit_Log, function(err, text, headers) end, 'POST', json.encode({username = "np",  avatar_url = "https://cdn.discordapp.com/attachments/928946993840132116/930453062403899472/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)

local Withdraw_Log = "https://discord.com/api/webhooks/960184023865491537/tX2zV_WEr3g50QRl7jmIzhiqyKRFX0UUNNzZP7UuOFw8zxZ20H8jw-TG8e3HPXGLp1sI"

RegisterServerEvent('rd-banking:withdraw_log')
AddEventHandler('rd-banking:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "255",
    ...
    PerformHttpRequest(Withdraw_Log, function(err, text, headers) end, 'POST', json.encode({username = "np",  avatar_url = "https://cdn.discordapp.com/attachments/928946993840132116/930453062403899472/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)