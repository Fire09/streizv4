...
RegisterNUICallback('drop', function(data, cb)
  closeGui()
  local notes = data.noteText
  local metaData = {
    ["NOTE"] = notes,
    ["_hideKeys"] = {'NOTE'},
  }
  TriggerEvent('player:receiveItem', 'notepadnote', 1, false, metaData)
  local ped = PlayerPedId()
  ClearPedTasks(ped)
  ClearPedTasksImmediately(PlayerPedId())
  TriggerEvent("playCancel")
