...
RegisterNetEvent('rd-eblips:server:registerSourceName')
AddEventHandler('rd-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('rd-eblips:server:registerPlayerBlipGroup')
AddEventHandler('rd-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('rd-eblips:server:removePlayerBlipGroup')
AddEventHandler('rd-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('rd-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('rd-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('rd-eblips:server:registerPlayerBlipGroup', source, 'police')
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('rd-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
