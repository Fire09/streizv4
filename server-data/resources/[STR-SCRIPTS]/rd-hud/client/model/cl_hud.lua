...
RegisterNUICallback('rd-hud:hudSetPreferences', function(data, cb)
    ...
    TriggerEvent("rd-preferences:setPreferences", data)
end)

AddEventHandler('rd-preferences:setPreferences', function(pData)
    exports["rd-ui"]:sendAppEvent('preferences', pData)
end)

local function toggleCompass()
    exports["rd-hud"]:sendAppEvent("hud.compass", {
...
AddEventHandler("rd-hud:watch", function()
...
AddEventHandler("rd-hud:watch:reset", function()
...
AddEventHandler('rd-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler("rd:voice:proximity", function(proximity)
    sendAppEvent("hud", {
        voiceRange = tonumber(proximity),
    })
end)

AddEventHandler("rd:voice:transmissionStarted", function(data)
    sendAppEvent("hud", {
...
AddEventHandler("rd:voice:transmissionFinished", function()
    sendAppEvent("hud", {
        voiceActive = false,
...
RegisterNetEvent("rd-hud:changeRange")
AddEventHandler("rd-hud:changeRange", function(proximity)
...
AddEventHandler("pd:deathcheck", function()
    isDead = not isDead
    exports["rd-hud"]:sendAppEvent("game", {
        isAlive = not isDead
    })
end)
...
RegisterNetEvent("rd-admin:currentDebug")
AddEventHandler("rd-admin:currentDebug", function(debugToggle)
...
AddEventHandler('rd-vehicles:autopilot:status', function (pStatus)
    isAutopilotEnabled = pStatus == 'started'
end)

AddEventHandler('rd-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('rd-boosting:client:setBoostCompletions')
AddEventHandler('rd-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('rd-weapons:client:setWeaponFireRate')
AddEventHandler('rd-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('rd-weapons:client:showWeaponFireRate')
AddEventHandler('rd-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('rd-ui:hud:forceShowMinimap', function(pForce)
...
AddEventHandler('rd-drifting:showDriftMode', function (pToggle)
    showDriftMode = pToggle
    setHudValue("showDriftMode", pToggle)
end)

AddEventHandler('rd-drifting:toggleDriftMode', function (pToggle)
    driftMode = pToggle
...
AddEventHandler("rd-drifting:remainingTireThread", function (pThread)
...
function getFuel()
    fuel = exports["rd-vehicles"]:CurrentFuel()
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                harnessDurability = exports["rd-vehicles"]:GetHarnessLevel(veh)
            end

            local degradation = exports["rd-vehicles"]:GetVehicleDegradation(veh)
...
Citizen.CreateThread(function()
    while true do
        ...
        if toggleParachuteOff and pState == -1 then
            ...
            exports["rd-hud"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["rd-hud"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("rd-island:onIsland", function(pState)
    onIsland = pState
end)
    ...
    while true do
        ...
        if (IsPedInAnyVehicle(player, false)) and GetIsVehicleEngineRunning(GetVehiclePedIsIn(player,false)) then
            ...
            if GetPedInVehicleSeat(veh, -1) == player then
                ...
                exports["rd-vehicles"]:SetVehicleFuel(veh, fRound(Fuel))
...
AddEventHandler('rd-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
