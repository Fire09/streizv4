...
RegisterNetEvent("rd-hud:pursuit_mode_a")
AddEventHandler("rd-hud:pursuit_mode_a",function()
...
RegisterNetEvent("rd-hud:pursuit_mode_b") -- Second Pursuit Mode
AddEventHandler("rd-hud:pursuit_mode_b",function()
	local ped = PlayerPedId()
	if (IsPedInAnyVehicle(PlayerPedId(), true)) then
		local veh = GetVehiclePedIsIn(PlayerPedId(),false)  
		local Driver = GetPedInVehicleSeat(veh, -1)
...
RegisterNetEvent("rd-hud:pursuit_mode_s")  -- Final Pursuit Mode for now anyways 0_o
AddEventHandler("rd-hud:pursuit_mode_s",function()
    local ped = PlayerPedId()
	if (IsPedInAnyVehicle(PlayerPedId(), true)) then
		local veh = GetVehiclePedIsIn(PlayerPedId(),false)  
		local Driver = GetPedInVehicleSeat(veh, -1)
...
RegisterNetEvent("rd-hud:pursuit_mode_off")
AddEventHandler("rd-hud:pursuit_mode_off",function()
local ped = PlayerPedId()
if (IsPedInAnyVehicle(PlayerPedId(), true)) then
	local veh = GetVehiclePedIsIn(PlayerPedId(),false)  
	local Driver = GetPedInVehicleSeat(veh, -1)
...
RegisterCommand('+evan_shift_pursuit_mode', function()
	...
	if IsPedSittingInAnyVehicle(ped) and IsVehicleModel(veh,defaultHash) or IsVehicleModel(veh,defaultHash2) or IsVehicleModel(veh,defaultHash3) or IsVehicleModel(veh,defaultHash4) or IsVehicleModel(veh,defaultHash5) or IsVehicleModel(veh,defaultHash6) or IsVehicleModel(veh,defaultHash7) then
		if not pursuitEnabled and not passedFirstMode then
			...
			pursuitEnabled = true
			TriggerEvent('rd-hud:pursuit_mode_b')
		elseif pursuitEnabled and not passedSecondMode then
			passedSecondMode = true
			InPursuitModeAPlus = true
			pursuitEnabled = false
			TriggerEvent('rd-hud:pursuit_mode_a') 
		elseif InPursuitModeAPlus and not passedThirdMode then
			passedThirdMode = true
			InPursuitModeB = true
			InPursuitModeAPlus = false
			TriggerEvent('rd-hud:pursuit_mode_s')
		elseif InPursuitModeB then
			passedFirstMode = false
			passedSecondMode = false
			passedThirdMode = false
			InPursuitModeAPlus = false
			TriggerEvent('rd-hud:pursuit_mode_off')
		end
	end
end)

Citizen.CreateThread(function()
    exports["rd-keybinds"]:registerKeyMapping("", "Pursuit Modes", "Change Mode", "+evan_shift_pursuit_mode", "")
end)


RegisterCommand('riddle', function(source, args, RawCommand)
	local ped = PlayerPedId()
