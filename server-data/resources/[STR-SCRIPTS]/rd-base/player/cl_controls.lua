STX.DataControls = STX.DataControls or {}
STX.Controls = STX.Controls or {}
STX.Controls.EventHolder = {}


STX.Controls.Current = {}
-- Current bind name and keys
STX.Controls.Default = {
  ["tokoptt"] = "CAPS",
  ["loudSpeaker"] = "-",
  ["distanceChange"] = "G",
  ["tokoToggle"] = "LEFTCTRL",
...
STX.Controls.secondaryBinds = {}

STX.Controls.Distcheck = {}
STX.Controls.Distcheck.use = {
	-- create fake id
...
STX.Controls.Distcheck.useSecondary = {
...
STX.Controls.Distcheck.useThird = {
	-- hospital garage
...
function STX.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(STX.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
function STX.DataControls.encodeSetBindTable(self, bindTable)
	...
	STX.DataControls.setBindTable(controlTable,true)
end

function STX.DataControls.toUpper(table)
...
function STX.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		STX.Controls.Current  = STX.DataControls.toUpper(STX.Controls.Default) 
		STX.DataControls.setSecondaryBindTable(STX.Controls.Current)
		TriggerServerEvent('rd-base:sv:player_control_set',STX.Controls.Current)
		STX.DataControls.checkForMissing()
	else
		if shouldSend then 
			STX.Controls.Current = STX.DataControls.toUpper(controlTable)
			STX.DataControls.setSecondaryBindTable(STX.Controls.Current)
			TriggerServerEvent('rd-base:sv:player_control_set',STX.Controls.Current)
			STX.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",STX.DataControls.getTableInKeyNumbers())
end

function STX.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#STX.Controls.secondaryBinds  do
				if STX.Controls.secondaryBinds[i] and STX.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				STX.Controls.secondaryBinds[i] = {}
				STX.Controls.secondaryBinds[i][1] = control1
				STX.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function STX.DataControls.getTableInKeyNumbers()
...
function STX.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(STX.Controls.Current) do
		if k == bindID then
			found = v
...
function STX.DataControls.distanceCall(distcheckName)
	local distanceTable = STX.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if STX.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-STX.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			STX.Controls.EventHolder[key] = GetGameTimer();
...
function STX.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(STX.Controls.Default) do
		if STX.Controls.Current[j] == nil then
			isMissing = true
			STX.Controls.Current[j] = h
		end
	end
...
function STX.DataControls.validEvent(event,id,eventID)
	local v = STX.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if STX.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-STX.Controls.EventHolder[eventKey])
	end

	if STX.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			STX.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if STX.Controls.Current["tokoptt"] then
			for i=0,#STX.Controls.events["general"] do
				local v = STX.Controls.events["general"][i]
				local keyString = STX.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#STX.Controls.secondaryBinds do
							if IsControlPressed(1,STX.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,STX.Controls.secondaryBinds[i][1]) then
								isConflicted = STX.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								STX.DataControls.validEvent(eventBind,idEvent,i)
							else
								STX.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#STX.Controls.secondaryBinds do
									if IsControlPressed(1,STX.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,STX.Controls.secondaryBinds[i][2]) then
										isConflicted = STX.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if STX.Controls.Current["tokoptt"] then
			for i=0,#STX.Controls.events["general-dist"] do
				local v = STX.Controls.events["general-dist"][i]
				local key = Keys[STX.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					STX.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("rd-base:cl:player_control")
AddEventHandler("rd-base:cl:player_control", function(controlTable)
	STX.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('rd-base:setcontrols')
AddEventHandler('rd-base:setcontrols', function()
	TriggerServerEvent('rd-base:sv:player_controls')
end)
