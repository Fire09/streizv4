...
AddEventHandler("mumbleConnected", function()
    ...
    TriggerEvent("rd:voice:state", true)
end)

AddEventHandler("mumbleDisconnected", function()
    print("Mumble: Disconnected")
    TriggerEvent("rd:voice:state", false)
end)

AddEventHandler("rd:voice:state", function (state)
    VoiceEnabled = state

    TriggerServerEvent("rd:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("rd:voice:transmission:state", serverID, context, true)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("rd:voice:transmission:state", serverID, context, false)
        end
...
function AddGroupToTargetList(group, context, pIsInVehicle)
    ...
    TriggerServerEvent("rd:voice:transmission:state", group, context, true, nil, pIsInVehicle)
end
...
function RemoveGroupFromTargetList(group, context)
    ...
    TriggerServerEvent("rd:voice:transmission:state", group, context, false)
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("rd:voice:proximity", proximity)
...
RegisterNetEvent("rd:voice:transmission:state")
AddEventHandler("rd:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
...
RegisterNetEvent("rd:voice:targets:player:add")
AddEventHandler("rd:voice:targets:player:add", AddPlayerToTargetList)

RegisterNetEvent("rd:voice:targets:player:remove")
AddEventHandler("rd:voice:targets:player:remove", RemovePlayerFromTargetList)

RegisterNetEvent("rd:voice:refresh")
AddEventHandler("rd:voice:refresh", RefreshConnection)

RegisterNetEvent("rd:voice:proximity:override")
AddEventHandler("rd:voice:proximity:override", function(pId, pMode, pRange, pPriority)
...
RegisterNetEvent("rd-voice:setTransmissionDisabled", function (pType, pDisabled)
...
Citizen.CreateThread(function()
    exports["rd-keybinds"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "Z")
    RegisterCommand("+cycleProximity", CycleVoiceProximity, false)
    RegisterCommand("-cycleProximity", function() end, false)

    exports["rd-keybinds"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
...
RegisterNetEvent("rd-voice:setVoiceProximity")
AddEventHandler("rd-voice:setVoiceProximity", function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("rd:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("rd:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
