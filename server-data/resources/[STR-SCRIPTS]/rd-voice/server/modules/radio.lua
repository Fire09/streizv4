...
function addPlayerToRadio(pServerId, pFrequency)
    ...
    for _, subscriber in ipairs(activeChannels[pFrequency]["subscribers"]) do
        TriggerClientEvent("rd:voice:radio:added", subscriber, pFrequency, pServerId)
    end
    ...
    TriggerClientEvent("rd:voice:radio:connect", pServerId, pFrequency, activeChannels[pFrequency]["subscribers"])
end
...
function removePlayerFromRadio(pServerId, pFrequency)
    ...
    if #activeChannels[pFrequency]["subscribers"] == 0 then
        ...
        for _, subscriber in ipairs(activeChannels[pFrequency]["subscribers"]) do
            TriggerClientEvent("rd:voice:radio:removed", subscriber, pFrequency, pServerId)
        end
    end
