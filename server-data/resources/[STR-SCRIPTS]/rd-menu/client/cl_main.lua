    ...
    for index, item in ipairs(pItems) do
        ...
        local success = pcall(function ()
            if entry and (not entry['isEnabled'] or entry['isEnabled'](pEntity, pContext)) then
                ...
                subMenu.title = exports["rd-i18n"]:GetStringSwap(subMenu.title)
                ...
                if index % MAX_MENU_ITEMS == 0 and index < (#pItems - 1) then
                    previous[MAX_MENU_ITEMS + 1] = {
                        id = "_more",
                        title = _L("menu-context-more", exports["rd-i18n"]:GetStringSwap("More")),
                        icon = "#more",
                        items = current
                    }
                    previous = current
        ...
        if IsMenuWanted(menuType, pContext.type) then
            for _, entry in ipairs(MenuEntries[menuType]) do
                if entry.isEnabled(pEntity, pContext) then
                    ...
                    menu.title = exports['rd-i18n']:GetStringSwap(menu.title);
...
--     local entity = exports['rd-target']:GetCurrentEntity()
...
--     exports["rd-keybinds"]:registerKeyMapping("Menu", "Show Menu", "+showFastMenu", "-showFastMenu", "F1")
-- end)
