...
function RefreshPeekList()
    ...
    Citizen.SetTimeout(250, function()
        ...
        exports["rd-ui"]:sendAppEvent('eye', {
            action = "refresh",
            payload = entries or {}
        })
    end)
...
function StartPeekin()
    ...
    NearZones = {}
    nearbyZones = exports["rd-polytarget"]:GetZones(GetEntityCoords(PlayerPedId())) or {}
    ...
    Citizen.CreateThread(function()
        ...
        while IsPeeking do
            ...
            if IsPeakActive and (IsControlJustReleased(0, 24) or IsDisabledControlJustReleased(0, 24)) then
                ...
                exports["rd-ui"]:openApplication("eye", {
                    action = "interact",
    ...
    exports["rd-ui"]:sendAppEvent('eye', { action = "peek", payload = { display = true, active = false } })
end
...
AddEventHandler("rd:target:changed", function(pTarget, pEntityType, pEntityOffset)
...
AddEventHandler('rd-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
AddEventHandler('rd-polyzone:exit', function(zoneName)
    if not PeekEntries['polytarget'][zoneName] then return end
...
RegisterUICallback("rd-ui:targetSelectOption", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })

    IsPeeking = false

    exports["rd-ui"]:closeApplication("eye")
...
AddEventHandler('rd-ui:phoneReady', function()
    RefreshPeekList()
end)
...
Citizen.CreateThread(function()
    exports["rd-keybinds"]:registerKeyMapping("", "Player", "Peek at Target", "+targetInteract", "-targetInteract", "LMENU")
end)
