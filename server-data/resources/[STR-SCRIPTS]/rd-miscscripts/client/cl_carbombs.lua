...
local function explodeVehicle(pEntity)
  exports['rd-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('rd-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['rd-taskbar']:taskBar(2500, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = STR.execute("rd-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
...
--[[   local remoteBomb = STR.execute('rd-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('rd-usableprops:defusePhoneBomb', number)
    TriggerEvent('rd-police:client:showTextPopup', { show = true, text = 'Cell Phone Found, Number: ' .. number })
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    TriggerServerEvent('rd-miscscripts:carbombs:foundBomb', NetworkGetNetworkIdFromEntity(pEntity), carBombMeta)
...
RegisterNetEvent("rd-miscscripts:carbombs:foundBombAll", function(pNetTable)
...
AddEventHandler("rd-inventory:itemUsed", function(name, info)
  ...
  local prompt = exports['rd-interface']:OpenInputMenu(elements)
  ...
  local progress = exports['rd-taskbar']:taskBar(2500, 'Planting car bomb...', true)
  ...
  local added = STR.execute('rd-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, 0, timeToComplete)
  TriggerEvent('inventory:removeItem', 'car_bomb', 1)
  
  return TriggerEvent('DoLongHudText', 'Successfully added car bomb to vehicle', 1)
end)

AddEventHandler('rd-miscscripts:carBombs:removeBomb', function (pData, pEntity)
    local metaData = STR.execute("rd-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
    local carBombMeta = metaData.carBombData or false
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    TriggerEvent('animation:PlayAnimation', 'kneel')
    TriggerServerEvent('rd-miscscripts:carbombs:removeBomb', myNetIdentifier)
...
--[[     exports['rd-interface']:openApplication('memorygame', {
      gameFinishedEndpoint = 'rd-miscscripts:carBombs:completeHacking',
