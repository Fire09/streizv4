...
local activeEvidence = CacheableMap(function (ctx)
    local success, evidence = RPC.execute("rd-evidence:fetchEvidence")
...
local collectEvidence = function (pCoords)
    ...
    RPC.execute("rd-evidence:clearEvidence", pCoords, evidence)
end

RegisterNetEvent('rd-evidence:clearCache', function ()
...
RegisterNetEvent('rd-evidence:startInspectView', function ()
...
RegisterNetEvent('rd-evidence:stopInspectView', function ()
...
RegisterCommand('-collectEvidence', function ()
    ...
    local hasGatheringKit = exports['rd-inventory']:hasEnoughOfItem('gatheringkit', 1, false)
    ...
    local finished = exports["rd-taskbar"]:taskBar(timer,"Picking Up Evidence","What?",true)
...
Citizen.CreateThread(function ()
    exports['rd-keybinds']:registerKeyMapping('', 'Evidence', 'Pick Up Evidence', '+collectEvidence', '-collectEvidence', 'E')
    ...
    while true do
        ...
        if canInspect and not InspectViewEnabled then
            ...
            TriggerEvent('rd-evidence:startInspectView')
        elseif not canInspect and InspectViewEnabled then
            idle = 100

            TriggerEvent('rd-evidence:stopInspectView')
        end
