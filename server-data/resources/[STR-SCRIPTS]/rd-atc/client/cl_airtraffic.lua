...
function UpdateAirTraffic(pAirspace)
    ...
    exports['rd-fx']:PlayEntitySound(PlayerPedId(), 'IDLE_BEEP', 'EPSILONISM_04_SOUNDSET')
end
...
function ShowAirTraffic()
    ...
    exports['rd-ui']:showContextMenu(elements)
end

function SetFlightData(pNetId)
    local result = exports['rd-ui']:OpenInputMenu({
    ...
    TriggerServerEvent("rd-atc:updateFlightData", pNetId, data)
    STR.execute('rd-atc:updateFlightData', pNetId, data)
end

function UpdateFlightData(pNetID, pData)
  --  print("riddle test airtraffic", AirTraffic[pNetID])
    if not AirTraffic[pNetID] then return end
    print("RIDDLE TEST ATC 4")
    AirTraffic[pNetID] = pData

    aircraftData.reset(pNetID)
end

AddEventHandler('rd-atc:setFlightData', function ()
...
RegisterUICallback('rd-atc:setFlightData', function (data, cb)
...
AddEventHandler('rd-atc:showAirTraffic', function ()
    if not HasRadarEnabled then return end
