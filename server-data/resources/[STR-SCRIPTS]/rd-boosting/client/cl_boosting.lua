            ...
            Citizen.CreateThread(function()
                while notEmailed do
                  ...
                  if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), setCoords) >= 100.0 then
                    ...
                    TriggerServerEvent('rd-boosting:pay_gne')
                    notEmailed = false
                  end
                end
              end)
...
RegisterCommand("lockpicktest", function()
  ...
  if targetVehicle == 0 then
    if vehicleHandle ~= nil and vehicleHandle ~= 0 and IsEntityAVehicle(vehicleHandle) then
        ...
        if CustomMinigame() == true then
            ...
            if(GetVehicleNumberPlateText(vehicleHandle) == placa)then
            if isVin then
              exports['rd-dispatch']:dispatchadd('10-99', 'Tracker Device Tampering - Grand Theft Auto', '227')
...
RegisterNetEvent("rd-boosting:open:boostingApp")
AddEventHandler('rd-boosting:open:boostingApp', function()
  TriggerServerEvent("boosting:getLevel")
  TriggerEvent("boosting:qbSpawn")
  TriggerEvent("open:boostingApp")
end)

RegisterNUICallback("rd-boosting:closeBoostingApp", function(data, cb)
  DoTimer(false)
...
RegisterNUICallback("rd-boosting:startContract", function(data, cb) 
  if not AlredyContract then
    if data.IsVin then
      ...
      if units + tonumber(data.props.ExtraVin) <= tonumber(gne) then
        if exports['rd-inventory']:hasEnoughOfItem('trackerdisabler', 1) then
...
RegisterNUICallback("rd-boosting:transferContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  TriggerServerEvent("boosting:TransferContract", data)
  cb({})
end)

RegisterNUICallback("rd-boosting:updateTime", function(data, cb) 
...
RegisterNUICallback("rd-boosting:DeclineContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
...
RegisterNUICallback("rd-boosting:cancelContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  sendAppEvent('SetDisableButtons', {enableButtons = false})
  TriggerServerEvent("boosting:CancelContract", data)
...
RegisterNUICallback("rd-boosting:JoinQueue", function(data, cb) 
  ...
  if IsInQueue then
    ...
    TriggerEvent("signIntoJob", "boosting")
  else
...
RegisterNUICallback("rd-boosting:BennysItem", function(data, cb) 
  if pendingDelivery == false then
    if data.ToPay <= tonumber(gne) then
      TriggerServerEvent('rd-boosting:removeGNE', data.ToPay, exports['isPed']:isPed('cid'))
...
RegisterNUICallback("rd-purchaseGNE", function(data, cb) 
  print("RAGGSYY TEST")
...
RegisterNUICallback("rd:sendgne", function(data, cb)
...
RegisterNetEvent("rd-boosting:client:UseComputer")
AddEventHandler("rd-boosting:client:UseComputer" , function()
  if exports['rd-boosting']:pCanUseLaptop() then
    CanScratchVehicle = false
    RemoveBlip(dropZoneblip)
    FreezeEntityPosition(PlayerPedId(),true)
    TriggerEvent("animation:PlayAnimation", 'type')
    local finished = exports['rd-taskbar']:taskBar(1500, 'Opening Laptop')
    if (finished == 100) then
      Citizen.Wait(250)
      TriggerEvent("animation:PlayAnimation", 'type')
      local finished = exports['rd-taskbar']:taskBar(7000, 'Connection to network...')
      if (finished == 100) then
        Citizen.Wait(250)
        TriggerEvent("animation:PlayAnimation", 'type')
        local finished = exports['rd-taskbar']:taskBar(13000, "Wiping Online Paperwork...")
        if finished == 100 then
...
RegisterNetEvent('rd-boosting:finish_scratch')
AddEventHandler('rd-boosting:finish_scratch', function()
  local veh = pVeh
  local playerped = PlayerPedId()
  ...
  TriggerEvent('animation:PlayAnimation', 'kneel')
    ...
    local finished = exports['rd-taskbar']:taskBar(10000, 'Scratching Vin')
    if (finished == 100) then
