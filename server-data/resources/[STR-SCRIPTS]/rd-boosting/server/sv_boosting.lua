...
RegisterServerEvent('rd-boosting:removeGNE')
AddEventHandler('rd-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterServerEvent('rd-boosting:pay_gne')
AddEventHandler('rd-boosting:pay_gne', function()
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
...
RegisterServerEvent('rd-boosting:giveGNE')
AddEventHandler('rd-boosting:giveGNE', function(amount)
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
