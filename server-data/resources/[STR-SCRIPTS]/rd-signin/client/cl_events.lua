...
RegisterNetEvent('job:doccount')
AddEventHandler('job:doccount', function(pAmount)
	curDoc = pAmount
end)
...
exports("OnlineDoc", OnlineDoc)

RegisterUICallback("rd-signin:handler", function(data, cb)
  local eventData = data.key
  if eventData.sign_in then
    TriggerServerEvent("rd-signin:duty", eventData.job)
...
RegisterUICallback("rd-signin:spawnVehicle", function(data, cb)
  local eventData = data.key
...
AddEventHandler("rd-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  exports["rd-ui"]:showContextMenu(MenuData[name])
end)

AddEventHandler("rd-signin:legalAid", function(pArgs, pEntity, pContext)
  TriggerEvent("DoLongHudText", "You now appear as a legal aid.")
  RPC.execute('rd-gov:dojApp:setStatus', "legal_aid", "Available")
end)
