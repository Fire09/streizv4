...
Citizen.CreateThread(function()
    Elevators = RPC.execute('rd-doors:elevators:fetch')
    ...
    for _, elevator in ipairs(Elevators) do
        ...
        for floorId, floor in ipairs(floors) do
            ...
            local data, lib = zone.options.data or {}, zone.target and 'rd-polytarget' or 'rd-polyzone'

            data.floorId = floorId
            data.elevatorId = elevator.id

            exports[lib]:AddBoxZone("rd-doors:elevator", zone.center, zone.width, zone.length, zone.options)
        end
    end

    exports['rd-interact']:AddPeekEntryByPolyTarget('rd-doors:elevator', {
        {
            id = "elevatorPrompt",
            event = "rd-doors:elevator:prompt",
            icon = "chevron-circle-up",
            label = "Elevator"
        }}, { distance = { radius = 1.5 } })
end)

AddEventHandler('rd-doors:elevator:prompt', function(pParameters, pEntity, pContext)
    local data = pContext.zones and pContext.zones['rd-doors:elevator']

    if not data or not Elevators[data.elevatorId] then return end

    local hasHackingDevice = exports["rd-inventory"]:hasEnoughOfItem('elevatorhackingdevice', 1, false, true)
...
function OpenElevatorMenu(pElevator, pCurrentFloor, pBreachingDevice)
    ...
    for floorId, floor in ipairs(elevator.floors) do
        ...
        elements[#elements+1] = {
            ...
            action = (not isCurrentFloor and not isRestricted) and 'rd-doors:elevator:teleport' or '',
            key = floor.teleport,
        ...
        if hasAccess then
            access[#access+1] = {
                ...
                action = 'rd-doors:elevator:access',
    ...
    if not hasAccess and pBreachingDevice then
        elements[#elements+1] = {
            ...
            action = 'rd-doors:elevator:hackControlPanel',
            key = { type = "elevator", id = pElevator }
        }
    end

    exports['rd-ui']:showContextMenu(reverse(elements))
end
...
RegisterNetEvent('rd-doors:elevators:updateState')
AddEventHandler('rd-doors:elevators:updateState', function (pElevatorId, pFloorId, pRestricted, pForceUnlock)
...
RegisterUICallback('rd-doors:elevator:teleport', function (data, cb)
    ...
    local finished = exports["rd-taskbar"]:taskBar(time, "Waiting for the Elevator", false)
...
RegisterUICallback('rd-doors:elevator:access', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    TriggerServerEvent('rd-doors:change-elevator-state', elevatorId, floorId, not locked)
end)

RegisterUICallback('rd-doors:elevator:hackControlPanel', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local id = data.key.id
    local accessType = data.key.type

    local hackId = RPC.execute('rd-doors:startAccessHack', accessType, id)

    if not hackId then return TriggerEvent('DoLongHudText', 'Unable to establish connection', 2) end

    exports['rd-ui']:openApplication('memorygame', {
        gameFinishedEndpoint = 'rd-doors:elevator:completeHacking',
...
RegisterUICallback('rd-doors:elevator:completeHacking', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local success = data.success
    local hackId = data.parameters.hackId

    RPC.execute('rd-doors:completeAccessHack', success, hackId)
