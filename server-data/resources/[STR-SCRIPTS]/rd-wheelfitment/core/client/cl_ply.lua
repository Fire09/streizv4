DecorRegister("rd-wheelfitment_applied", 2)
DecorRegister("rd-wheelfitment_w_width", 1)
DecorRegister("rd-wheelfitment_w_fl", 1)
DecorRegister("rd-wheelfitment_w_fr", 1)
DecorRegister("rd-wheelfitment_w_rl", 1)
DecorRegister("rd-wheelfitment_w_rr", 1)
--[[
cl_ply.lua
]] -- #[Local Variables]#--
...
function SyncWheelFitment()
    if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            if not DecorExistOn(plyVeh, "rd-wheelfitment_applied") then
                DecorSetBool(plyVeh, "rd-wheelfitment_applied", true)
            end

            DecorSetFloat(plyVeh, "rd-wheelfitment_w_width", roundNum(GetVehicleWheelWidth(plyVeh), 2))
            DecorSetFloat(plyVeh, "rd-wheelfitment_w_fl", roundNum(GetVehicleWheelXOffset(plyVeh, 0), 2))
            DecorSetFloat(plyVeh, "rd-wheelfitment_w_fr", roundNum(GetVehicleWheelXOffset(plyVeh, 1), 2))
            DecorSetFloat(plyVeh, "rd-wheelfitment_w_rl", roundNum(GetVehicleWheelXOffset(plyVeh, 2), 2))
            DecorSetFloat(plyVeh, "rd-wheelfitment_w_rr", roundNum(GetVehicleWheelXOffset(plyVeh, 3), 2))
            ...
            TriggerServerEvent("rd-wheelfitment_sv:saveWheelfitment", plate, fitment)
        ...
        RPC.execute("rd-wheelfitment_sv:setIsWheelFitmentInUse", false)
    end
end
...
function checkVehicleFitment()
    ...
    for _, veh in ipairs(vehicles) do
        ...
        if isNear(plyPos, GetEntityCoords(veh), 150) then
            if DecorExistOn(veh, "rd-wheelfitment_applied") then
                vehiclesToCheckFitment[#vehiclesToCheckFitment + 1] = {
                    vehicle = veh,
                    w_width = DecorGetFloat(veh, "rd-wheelfitment_w_width"),
                    w_fl = DecorGetFloat(veh, "rd-wheelfitment_w_fl"),
                    w_fr = DecorGetFloat(veh, "rd-wheelfitment_w_fr"),
                    w_rl = DecorGetFloat(veh, "rd-wheelfitment_w_rl"),
                    w_rr = DecorGetFloat(veh, "rd-wheelfitment_w_rr")
                }
            end
        end
    end
...
Citizen.CreateThread(function()
    isWheelFitmentInUse = RPC.execute("rd-wheelfitment_sv:getIsWheelFitmentInUse")
    isPlyWhitelisted = RPC.execute("rd-wheelfitment_sv:checkIfWhitelisted")

    exports["rd-polyzone"]:AddBoxZone("rd-wheelfitment:zone1", vector3(136.0, -3030.36, 6.52), 7.8, 4.2, {
        name="rd-wheelfitment:zone1",
        heading=1,
        --debugPoly=false
      })
end)
...
AddEventHandler("rd-polyzone:enter", function(zone, data)
    if zone ~= "rd-wheelfitment:zone1" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
                ...
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if IsControlJustReleased(1, 38) then -- Key: E
                            ...
                            RPC.execute("rd-wheelfitment_sv:setIsWheelFitmentInUse", true)
...
RegisterNetEvent('rd-fitment:devaleks',function()
    devwheelMenu ()
end)


AddEventHandler("rd-polyzone:exit", function(zone)
    if zone ~= "rd-wheelfitment:zone1" then
        return
    end
    inZone = false
end)
...
AddEventHandler("rd-admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)

-- #[Event Handlers]#--
RegisterNetEvent("rd-wheelfitment_cl:applySavedWheelFitment")
AddEventHandler("rd-wheelfitment_cl:applySavedWheelFitment", function(plyVeh, VehicleWheel)
    print("riddlev TEST", plyVeh, VehicleWheel)
    ...
    DecorSetFloat(plyVeh, "rd-wheelfitment_w_width", wheelFitments.width)
    DecorSetFloat(plyVeh, "rd-wheelfitment_w_fl", wheelFitments.fl)
    DecorSetFloat(plyVeh, "rd-wheelfitment_w_fr", wheelFitments.fr)
    DecorSetFloat(plyVeh, "rd-wheelfitment_w_rl", wheelFitments.rl)
    DecorSetFloat(plyVeh, "rd-wheelfitment_w_rr", wheelFitments.rr)

    performVehicleCheck = true
    checkVehicleFitment()
end)

RegisterNetEvent("rd-wheelfitment_cl:forceMenuClose")
AddEventHandler("rd-wheelfitment_cl:forceMenuClose", function()
    if isMenuShowing then
        local plyPed = PlayerPedId()
        local plyVeh = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("leavefitment", function()
    TriggerEvent("rd-wheelfitment_cl:forceMenuClose")
end)