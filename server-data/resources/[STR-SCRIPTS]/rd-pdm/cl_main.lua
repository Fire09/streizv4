...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("rd-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("rd-pdm:testdrive")
AddEventHandler("rd-pdm:testdrive", function()
	if exports["rd-business"]:IsEmployedAt("pdm") or #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 then
	...
	if not DoesEntityExist(veh) then
		...
		Citizen.Wait(100)
		TriggerEvent("vehicle:keys:addNew",vehplate, plate)
...
RegisterNetEvent("rd-pdm:EnableBuy")
AddEventHandler("rd-pdm:EnableBuy", function()
	if #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["rd-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("rd-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("rd-pdm:BuyEnabledCL")
AddEventHandler("rd-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("rd-pdm:PDMCommission")
AddEventHandler("rd-pdm:PDMCommission", function(newAmount)
	if eexports["rd-business"]:IsEmployedAt("pdm") or #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("rd-pdm:ReturnPDMTTable")
AddEventHandler("rd-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('rd-pdm:ChechMoney',name, model, price)
	commissionbuy = (baseprice * commission / 100)
	Citizen.Wait(100)
	exports['rd-ui']:hideInteraction()
	TriggerServerEvent('rd-banking:business_money', price, 'pdm', 'add')
end

RegisterCommand("aptalibne", function()
	OpenCreator()
end)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["rd-business"]:IsEmployedAt("pdm") then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("rd-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('rd-pdm:BuyVehicle', plate, name, vehicle, price, VehicleProps)
			DespawnSaleVehicles()
			SpawnSaleVehicles()
			Citizen.Wait(100)
			exports['rd-ui']:hideInteraction()
...
RegisterNetEvent("rd-pdm:FailedPurchase")
AddEventHandler("rd-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "s-class" or this == "a-class" or this == "b-class" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('rd-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["rd-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["rd-business"]:IsEmployedAt("pdm") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('rd-pdm:PDMCommission', amount)
		else
