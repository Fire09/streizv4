...
RegisterServerEvent("rd-fx:setSteamId")
AddEventHandler("rd-fx:setSteamId", function()
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("rd-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("rd-fx:smoke:grenade")
AddEventHandler("rd-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("rd-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
