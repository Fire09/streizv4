...
RegisterNetEvent("rd-editor:buildName")
AddEventHandler("rd-editor:buildName", function(name, objects)
    if objects then
...
RegisterNetEvent("rd-editor:destroyName")
AddEventHandler("rd-editor:destroyName", function(name)
    cleanUpArea(name)
...
function saveObjects(name)
    ...
    local changedTable = STR.execute("objects:saveObjects",dataToSend)
    CURRENT_DELETED = {}
...
function loadObjects(name)
    TriggerServerEvent("objects:GetObjectEditor", name)
    local data = STR.execute("objects:getObjects")
