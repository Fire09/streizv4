	...
	for i = 1, #BanList, 1 do
		...
		then
			...
			if (tonumber(BanList[i].expire)) > os.time() then
				...
				if tempsrestant >= 1440 then
					...
					local txtminutes = math.ceil(minutes)
						done("You are banned, you may appeal it at riddlev#1337 | Ban ID: " .. BanList[i].banid .. " | Reason: " .. BanList[i].reason .." | Unban Date: "..  os.date('%Y-%m-%d', BanList[i].expire))					
						CancelEvent()
						break
			...
			elseif (tonumber(BanList[i].expire)) < os.time() and tonumber(BanList[i].expire) > 0 then
				--exports['rd-admin']:deletebanned(license)
				break
			elseif tonumber(BanList[i].expire) <= 0 then
				done("You are banned, you may appeal it at riddlev#1337 | Ban ID: " .. BanList[i].banid .. " | Reason: " .. BanList[i].reason .." | Unban Date: Permanent") 
				CancelEvent() 
				break 
			end
		end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [rd-queue] Disabling hardcap ^7")
