...
RegisterNUICallback('rd-uix:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'rd-uix:inputResponse') then return end
...
RegisterNUICallback('rd-uix:inputResponse', function(data, cb)
    ...
    exports['rd-uix']:closeApplication('textbox')
end)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['rd-uix']:openApplication('textbox', {
        callbackUrl = 'rd-uix:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
