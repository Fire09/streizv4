...
var ShowroomFunctions = {
    ...
    GetCars: function () {
        $.post('https://rd-showroom/rd-ui:showroomChangeCar', JSON.stringify({ model: cars[selectedId].model }), function (cb) {
...
$(document).ready(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.meta.message = "done") {
            ...
            $.post('https://rd-showroom/rd-ui:application-closed', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
$(document).on("click", "#purchase", function () {
    ...
    $(".showroom-wrapper").hide();
    $.post('https://rd-showroom/rd-ui:application-closed', JSON.stringify({ name: "showroom" }));
    $.post('https://rd-showroom/showroomPurchaseCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
...
$(document).on("click", "#testdrive", function () {
    ...
    $(".showroom-wrapper").hide();
    $.post('https://rd-showroom/rd-ui:application-closed', JSON.stringify({ name: "showroom" }));
    $.post('https://rd-showroom/showroomTestDriveCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
