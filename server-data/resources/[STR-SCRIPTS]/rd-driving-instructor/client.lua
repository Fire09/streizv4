    ...
    while isInstructorMode do
      ...
      if isInVehicle then
        ...
        if playerVeh ~= 0 and isVehicleAllowed(playerVeh) then -- Check if we weren't in a vehicle before but now are
          -- if exports["rd-keys"]:hasKey(GetVehicleNumberPlateText(playerVeh)) then -- Only run on vehicles we have keys for
          if exports['rd-vehicles']:HasVehicleKey(playerVeh) then
            isInVehicle = true
...
RegisterCommand("instructor", function()
