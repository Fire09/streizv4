...
RegisterNetEvent("rd-usableprops:megaphone", function()
  ...
  while true do
    ...
    if usingMegaphone then
      ...
      while usingMegaphone and not IsEntityDead(ped) and (GetVehiclePedIsIn(ped) == 0) and IsEntityPlayingAnim(ped, "amb@world_human_mobile_film_shocking@female@base", "base", 3) do
        if not hasSet then
          TriggerEvent('rd:voice:proximity:override', "megaphone", 3, 75.0, 2)
          TriggerServerEvent("rd:voice:transmission:state", -1, 'megaphone', true, 'megaphone')
          hasSet = true
        end
        Wait(0)
      end
      ...
      TriggerEvent('rd:voice:proximity:override', "megaphone", 3, -1, -1)
      TriggerServerEvent("rd:voice:transmission:state", -1, 'megaphone', false, 'megaphone')
    end
  end
end)