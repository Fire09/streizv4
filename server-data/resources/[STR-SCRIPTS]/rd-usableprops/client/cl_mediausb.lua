AddEventHandler("rd-inventory:itemUsed", function(item, info, inventoryName, slot)
  ...
  if #results < 10 then
    context[#context + 1] = {
      title = "Add New Entry",
      action = "rd-usableprops:addUSBEntry",
      icon = "plus",
  ...
  for _, entry in pairs(results) do
    ...
    context[#context + 1] = {
      ...
      children = {
        { title = "View (opens in browser)", icon = "eye", description = url, action = "rd-usableprops:viewUSBEntry", key = url },
        {
          title = "Delete",
          icon = "trash",
          children = {
            {
              title = "Are you sure?",
              icon = "check-square",
              action = "rd-usableprops:deleteUSBEntry",
...
RegisterUICallback("rd-usableprops:addUSBEntry", function(pData, pCb)
...
--  RPC.execute("rd-usableprops:saveUSB", data.meta.id, prompt.title, prompt.url)
  TriggerEvent("inventory:updateItem", data.item, data.slot, json.encode(data.meta));
end)

RegisterUICallback("rd-usableprops:viewUSBEntry", function(pData, pCb)
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  local url = pData.key
  -- TriggerEvent("rd-ui:openUrl", url)
end)

RegisterUICallback("rd-usableprops:deleteUSBEntry", function(pData, pCb)
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  local data = pData.key
