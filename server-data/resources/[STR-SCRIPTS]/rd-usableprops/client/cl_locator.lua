...
AddEventHandler('rd-inventory:itemUsed', function(item, info, inventoryName, slot)
  ...
  if item == 'gps_locator' then
    ...
    local locations = RPC.execute('rd-usableprops:getLocatorPositions', trackerID)
...
function checkForTrackerId(item, meta, slot)
  ...
  local prompt = exports['rd-ui']:OpenInputMenu({ { label = 'Enter ID', name = 'id' } }, function(values)
