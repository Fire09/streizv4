        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["rd-taskbar"]:taskBar(15000, _L("beekeeping-installing", "Installing beehive"), false, true, false, false, nil, 5.0, PlayerPedId())
                    ClearPedTasks(PlayerPedId())
                    if finished == 100 then
                        RPC.execute("rd-beekeeping:installHive", pCoords, pHeading)
...
AddEventHandler("rd-beekeeping:checkBeehive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    showHiveMenu(hiveId)
end)

RegisterUICallback('rd-beekeeping:addQueen', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["rd-taskbar"]:taskBar(15000, _L("beekeeping-adding-queen-bee", "Adding Queen bee"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("rd-beekeeping:addQueen", data.key.id)
        TriggerEvent("inventory:removeItem", "beequeen", 1)
    end
end)

RegisterUICallback('rd-beekeeping:removeHive', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["rd-taskbar"]:taskBar(15000, _L("beekeeping-removing", "Removing"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getReady = getHiveReadyPercent(getHiveById(data.key.id)) > 20.0
        local success = RPC.execute("rd-beekeeping:removeHive", data.key, getReady)
        if not success then
...
AddEventHandler("rd-beekeeping:harvestHive", function(pContext, pEntity)
    local hiveId = getHiveId(pEntity)
    if not hiveId then return end
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["rd-taskbar"]:taskBar(5000, _L("beekeeping-harvesting", "Harvesting"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("rd-beekeeping:harvestHive", hiveId)
    end
end)

AddEventHandler("rd-polyzone:enter", function(zone, data)
    if zone == "rd-beekeeping:bee_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            local result = RPC.execute("rd-beekeeping:getBeehives")
        end
    end
end)
...
RegisterNetEvent('rd-beekeeping:trigger_zone')
AddEventHandler('rd-beekeeping:trigger_zone', function (type, pData)
    --Update all hives
    if type == 1 then
...
function showHiveMenu(pHiveId)
    ...
    if getStageFromPercent(harvest) < 2 and not hive.has_queen then
        context[#context+1] = {
            ...
            key = { id = pHiveId },
            action = 'rd-beekeeping:addQueen',
            description = _L("beekeeping-add-queen-desc", 'Make the hive happy.'),
            disabled = not exports["rd-inventory"]:hasEnoughOfItem("beequeen", 1, false)
        }
    end
    ...
    if harvest >= 95 or myjob == "police" or myjob == "judge" then
        context[#context+1] = {
            ...
            key = { id = pHiveId },
            action = 'rd-beekeeping:removeHive',
        }
    end
