...
RegisterNetEvent("rd-scenes:getScenes")
AddEventHandler("rd-scenes:getScenes", function()
    local src = source
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))

    if not data then
        TriggerClientEvent("rd-scenes:allScenes", src, {})
        return
    end
    ...
    TriggerClientEvent("rd-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("rd-scenes:addScene")
AddEventHandler("rd-scenes:addScene", function(pData)
    local src = source
    ...
    TriggerClientEvent("rd-scenes:refreshScenes", -1, data[1].id, data[1])
end)

RegisterNetEvent("rd-scenes:deleteScene")
AddEventHandler("rd-scenes:deleteScene", function(pCoords)
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("rd-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
