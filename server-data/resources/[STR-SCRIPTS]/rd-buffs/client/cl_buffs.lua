...
AddEventHandler("rd-buffs:stamina", function(pEnabled, pPercent)
...
AddEventHandler("rd-buffs:setJobLuckMultiplier", function(pJobLuckMultiplier)
  jobLuckMultiplier = 1 + pJobLuckMultiplier + 0.0
  TriggerServerEvent("rd-buffs:setBuffValue", "jobluck", jobLuckMultiplier)
end)
...
AddEventHandler("rd-buffs:setAlertLevelMultiplier", function(pAlertLevelMultiplier)
...
AddEventHandler("rd-buffs:inDoubleBuffZone", function(pBool)
  ...
  Citizen.CreateThread(function()
    ...
    while isSitting and inRestaurantBuffZone do
      if minuteCount >= 5 then
        ...
        TriggerEvent("rd-buffs:applyBuff", "restaurant_double", {{ buff = "double", percent = percent }})
      end
      Citizen.Wait(60000)
