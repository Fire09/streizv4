  ...
  ["resfooditem"] = {
    ...
    restaurantBaseBuff = {
      ["burger_shot"] = function(info)
        ...
        TriggerEvent("rd-buffs:applyBuff", "burger_shot", { { buff = "stressblock", percent = 1.0, timeOverride = 180 * 60000 } })
        TriggerEvent("hadsugar")
        return processInfo(info)
      end,
      ["rooster"] = function(info)
        TriggerEvent("rd-buffs:applyBuff", "rooster", { { buff = "jobluck", percent = 1.0 } })
        return processInfo(info)
      end,
      ["uwu_cafe"] = function(info)
        TriggerEvent("rd-buffs:applyBuff", "uwu_cafe", { { buff = "strength", percent = 0.5, timeOverride = 60000 * 60 } })
        return processInfo(info)
      end,
      ["maldinis"] = function(info)
        TriggerEvent("rd-buffs:applyBuff", "maldinis", { { buff = "fullness", percent = 1.0 } })
        return processInfo(info)
      end,
    },
  },
...
AddEventHandler("rd-buffs:itemUsedSuccess", function(pSuccess)
...
AddEventHandler("rd-inventory:itemUsed", function(pItem, pInfo)
  ...
  itemUseSuccess = nil
  TriggerEvent("rd-inventory:attachPropPlayAnim", config.attachItem)
  ...
  TriggerEvent("rd-buffs:applyBuff", pItem, buffs)
