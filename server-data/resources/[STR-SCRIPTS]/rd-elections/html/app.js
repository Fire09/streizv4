...
document.onkeydown = function(data){
    if (data.which == 27){
        ...
        $.post('https://rd-elections/close')
    }
}


function SetupData(){
    $.post('https://rd-elections/getConfig', JSON.stringify({}), function(vCode){
...
$(document).on('click','.Vote', function(){
    if (selected != ''){
        $.post('https://rd-elections/voteForSomeone', JSON.stringify({id:selected}))
        $('.bottompart').hide()
