AddEventHandler("rd-vineyard:npcManager", function(p1)
  ...
  if p1.id == "start_harvest" then
    RPC.execute("rd-vineyard:startHarvest")
    return
  end
  if p1.id == "rent_atv" then
    local finished = exports["rd-taskbar"]:taskBar(math.random(5000, 10000), "Grabbing ATV")
    if finished ~= 100 then return end
    RPC.execute("rd-business:herowine:chargeCompany", 5000)
    local cid = exports["isPed"]:isPed("cid")
    local rentalData = RPC.execute("rd:vehicles:rentalSpawn", "verus", { x = -1926.68, y = 2065.61, z = 140.62 }, 256.71)
...
AddEventHandler("rd-vineyard:pickupBasket", function(p1, p2, p3)
  if p3 and p3.meta and p3.meta.data and p3.meta.data.metadata and p3.meta.data.metadata.type == "vineyard_grape_basket" then
    ...
    RPC.execute("rd-vineyard:pickupBasket", objectId, data)
  end
end)

Citizen.CreateThread(function()
  exports["rd-interact"]:AddPeekEntryByModel({ `prop_fruit_basket` }, {
    {
      id = "peek_prop_fruit_basket",
      event = "rd-vineyard:pickupBasket",
