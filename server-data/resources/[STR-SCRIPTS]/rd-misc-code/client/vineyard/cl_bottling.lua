RegisterUICallback("rd-vineyard:setupWineboxContainer", function(data, cb)
  ...
  if data.key.item == "vineyardwinebox" then
    RPC.execute("rd-restaurants:getTakeoutBox", {
...
AddEventHandler("rd-inventory:itemUsed", function(pItem, pData)
  ...
  exports["rd-ui"]:openApplication("textbox", {
    callbackUrl = "rd-vineyard:setupWineboxContainer",
...
AddEventHandler("rd-inventory:itemUsed", function(pItem, pData)
  ...
  local amount = exports["rd-inventory"]:getQuantity("wineglass", true, false)
  if amount == 0 then
