...
function repairCost(veh, health)
    if health < 1000.0 then
        ...
        if exports["rd-business"]:IsEmployedAt("drift_school") then
            return price * 0.5
        else
            return price
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if insideDriftSchool then
            ...
            if repairDist[1] < 5 and health < 1000.0 and veh ~= 0 and exports["rd-business"]:IsEmployedAt("drift_school") then
                ...
                if IsControlJustPressed(1, 38) then
                    TriggerServerEvent("rd-driftschool:takemoney", repairCost(veh, health))
                    ...
                    if not isPurchaseSuccessful then
                        ...
                        local finished = exports["rd-taskbar"]:taskBar(15000,"Fixing Vehicle",true)
            ...
            elseif testDriveDist[1] < 5 and exports["rd-business"]:IsEmployedAt("drift_school") then
                -- testdrive menu
...
RegisterNetEvent('rd-driftschool:tookmoney')
AddEventHandler('rd-driftschool:tookmoney', function(taken)
...
RegisterNUICallback('spawntestdrive', function(data, cb)
    ...
    if exports["rd-business"]:IsEmployedAt("drift_school") or dist[1] > 5 then
...
Citizen.CreateThread(function()
    while true do
        ...
        if insideDriftSchool and isExportReady then
            rank = exports["rd-business"]:IsEmployedAt("drift_school")
            Citizen.Wait(10000)
        end
    end
end)
