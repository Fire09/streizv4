Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['rd-interact']:AddPeekEntryByPolyTarget('rd-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "rd-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('rd-polychair:stopSitting')
    else
        ...
        TriggerEvent('rd-polychair:startSitting')
    end
    exports["rd-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('rd-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("rd-emotes:sitOnChair")
AddEventHandler("rd-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
