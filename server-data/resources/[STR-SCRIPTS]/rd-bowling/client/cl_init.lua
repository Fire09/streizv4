...
local function canUseLane(pLaneId)
    local info = (exports["rd-inventory"]:GetInfoForFirstItemOfName("bowlingreceipt") or { information = "{}" })
    local laneId = json.decode(info.information).lane

    return (exports['rd-inventory']:hasEnoughOfItem("bowlingreceipt", 1, false, true)) and (laneId == pLaneId)
end
...
Citizen.CreateThread(function()
    for k, v in pairs(lanes) do
        ...
        exports["rd-polyzone"]:AddBoxZone("rd-bowling:lane_"..k, v.pos, 1.8, 2.0, {
            heading=0,
            minZ=23.85,
            maxZ=27.85
        })
    ...
    exports["rd-polyzone"]:AddBoxZone("bowling_alley", vector3(743.95, -774.54, 26.34), 16.8, 30.4, {
        heading=0,
        minZ=23.85,
        maxZ=28.85
    })
    ...
    local bowlingVendor = exports["rd-npcs"]:RegisterNPC(data, "rd-bowling:bowlingVendor")
    exports["rd-npcs"]:EnableNPC(bowlingVendor)

    for k, v in pairs(lanes) do
        if (not v.enabled) then goto continuePeak end

        exports['rd-interact']:AddPeekEntryByPolyTarget('rd-bowling:lane_'..k, {{
            event = "rd-bowling:setupPins",
            id = "bowling_pins_"..k,
            icon = "bowling-ball",
    ...
    exports["rd-interact"]:AddPeekEntryByFlag({'isNPC'}, {{
            ...
            icon = "bowling-ball",
            event = 'rd-bowling:handleVendorPurchase',
...
AddEventHandler('rd-bowling:handleVendorPurchase', function(pParams)
    ...
    for itemId, item in pairs(options) do
        if (itemId == 'bowlingreceipt') then
            ...
            for k, v in ipairs(lanes) do
                table.insert(lanesSorted, { title = "Lane #" .. k, action = "rd-ui:lanePurchase", key = k, disabled = not v.enabled })
            end
            ...
            data[#data + 1] = {
                ...
                key = itemId,
                children = {
                    { title = _L("bowling-ui-confirmpurchase", "Confirm Purchase"), action = "rd-ui:bowlingPurchase", key = itemId },
                },
            }
        end
    end
...
RegisterUICallback("rd-ui:bowlingPurchase", function(data, cb)
    local isLane = type(data.key) == "number"

    local success, message = RPC.execute("rd-bowling:purchaseItem", data.key, isLane)
    if not success then
...
RegisterUICallback("rd-ui:lanePurchase", function(data, cb)
    local isLane = type(data.key) == "number"
    TriggerServerEvent('rd-bowling:lanePurchase', data.key, isLane) 
        cb({ data = {}, meta = { ok = success, message = message } })
    cb({ data = {}, meta = { ok = true, message = "done" } })
end)

AddEventHandler('rd-bowling:setupPins', function(pParameters, pEntity, pContext)
...
AddEventHandler('rd-inventory:itemUsed', function(pItemId)
