...
RegisterNetEvent("rd-camera:client:use-camera", function()
    if not active then
        active = true
                    ...
                    while active and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and true do
                        ...
                        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
                            if not presstake then
                                presstake = true
                              --  local hook = RPC.execute("rd-camera:server:webhook")
                                local hook = "https://discord.com/api/webhooks/1108028143173902366/6gMA5WTKDmdDpswycefQjCfFwdp3HnWsyGkd9adz3kbUcax6aY1h4CXHbttQurkMplHK"
                                    if hook then
                                        exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                                                                local image = json.decode(data)
                                                                FullClose()
                                                              local photoUrl = image.attachments[1].proxy_url
                                                              local metaData = {
                                                                ["URL"] = photoUrl,
                                                                ["_hideKeys"] = {'URL'},
                                                              }
                                                              TriggerEvent('player:receiveItem', 'npolaroid_photo', 1, false, metaData)
                                                              TriggerEvent("inventory:removeItem", "npolaroid_paper", 1)
                                                        end)						
                                    end
                                            end
                                        end
...
RegisterNetEvent("rd-camera:client:use-photo", function(url)
...
AddEventHandler("rd-inventory:itemUsed", function(item, info, information)
	if item ~= "npolaroid_camera" then return end
    local checkPaper = exports["rd-inventory"]:hasEnoughOfItem("npolaroid_paper", 1)
    if checkPaper then
    TriggerEvent("rd-camera:client:use-camera")
    else
        TriggerEvent("DoLongHudText", "Film is required to take photos.", 2)
    end
  end)

  AddEventHandler("rd-inventory:itemUsed", function(item, info)
    if item == "npolaroid_photo" then
        data = json.decode(info)
        local url = data.URL
        TriggerEvent("rd-camera:client:use-photo", url)
    end
  end)
