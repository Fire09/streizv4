...
AddEventHandler('rd-gopro:activateVRChair', function ()
  density = 0.0
  print('pausing polulation', density)
end)
...
AddEventHandler('rd-density:disable', function (pDensity, pRogue)
...
Citizen.CreateThread(function()
    while true do
      ...
      if #toDelete > 0 then
        TriggerServerEvent('rd:peds:rogue', toDelete)
      end
...
RegisterNetEvent('rd:peds:rogue:delete')
AddEventHandler('rd:peds:rogue:delete', function(pNetId)
...
RegisterNetEvent('rd:peds:decor:set')
AddEventHandler('rd:peds:decor:set', function (pNetId, pType, pProperty, pValue)
  local entity = NetworkGetEntityFromNetworkId(pNetId)
