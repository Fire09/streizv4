...
RegisterNUICallback('rd-ui:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'rd-ui:inputResponse') then return end
...
RegisterNUICallback('rd-ui:inputResponse', function(data, cb)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['rd-ui']:openApplication('textbox', {
        callbackUrl = 'rd-ui:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
