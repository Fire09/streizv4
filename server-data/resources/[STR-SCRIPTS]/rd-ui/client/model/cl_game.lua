...
-- AddEventHandler('rd-config:configReady', function()
--     loadPublicData()
-- end)

-- function loadPublicData()
--     local showWatermark = exports['rd-config']:GetMiscConfig('hud.watermark')
--     local showroomPurchase = exports['rd-config']:GetMiscConfig('showrooms.catalog.purchase')
--     local skipMdwProfileCheck = exports['rd-config']:GetMiscConfig('mdw.profiles.skip')
...
--         }
--         SendUIMessage({ source = "rd-nui", app = "character", data = character });
        
--         local endpointData = exports["rd-fiber"]:GetGameDetails()
--         SendUIMessage({ source = "rd-nui", app = "game", data = endpointData });

--         Citizen.Wait(5000)

--         TriggerEvent('rd-ui:phoneReady')
--         TriggerServerEvent('rd-ui:phoneReady')
--     end)
-- end
...
-- -- AddEventHandler("rd-ui:hud:values", function(values)
-- --   -- sendAppEvent("hud", {
-- --   --     food = values["hunger"],
...
AddEventHandler("rd-ui:openUrl", function(pUrl)
  --  os.execute("am start -n com.android.browser/com.android.browser.BrowserActivity "pUrl);
  os.execute('start '.. pUrl ..'')
end)