...
AddEventHandler("rd-inventory:itemUsed", function (itemId, itemInfo)
...
function spawn(coordinates)
  ...
  local location = GetEntityCoords(ply, 0) 
    --local location = exports["rd-housing"]:getCurrentLocation()
    if location then
    ...
    TriggerServerEvent("rd-storage:prepareStorage", isSelecting, cid, coordinates, heading)
    TriggerServerEvent('server-remove-item', cid, isSelecting, 1)
    stopSelecting()
end

function canPlaceInHouse (location)
    local data = RPC.execute("rd-objects:getObjects", location)
    local count = 0
    local crates = {}
