...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("rd-racing:joinedRace", src, Races[id])
    TriggerClientEvent("rd-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("rd-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("rd-racing:updatePositionInfo")
AddEventHandler("rd-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("rd-racing:updatePosition", src, position)
end)
...
RPC.register("rd-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("rd-racing:addedPendingRace", -1, options)
    TriggerClientEvent('rd-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("rd-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("rd-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("rd-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("rd-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("rd-racing:leftRace", src)
        TriggerClientEvent("rd-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("rd-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("rd-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("rd-racing:leftRace", src)
    TriggerClientEvent("rd-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("rd-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("rd-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("rd-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("rd-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("rd-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("rd-racing:startRace", v.serverId, race, countdown * 1000)
            TriggerClientEvent("rd-racing:api:currentRace", v.serverId, race)
        else
            local user = exports['rd-base']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("rd-racing:endRace", function(src)
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("rd-racing:removedActiveRace", -1, raceId)
end)

RPC.register("rd-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("rd-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("rd-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("rd-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("rd-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("rd-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("rd-racing:dnfRace", v.serverId)
            end
        end
