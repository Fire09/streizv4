...
function createPendingRace(id, options)
    ...
    RPC.execute("rd-racing:createPendingRace", id, options)
end
...
function startRace(countdown)
    ...
    for k, v in pairs(pendingRaces) do
        ...
        if v.owner == characterId then
            ...
            RPC.execute("rd-racing:startRace", v.id, countdown or v.countdown, v.owner)
            return
        end
    end
end
...
function endRace()
    ...
    if curRace then
        RPC.execute("rd-racing:endRace")
        print("ENDING RACE 1")
    else
        RPC.execute("rd-racing:leaveRace")
    end
end

function joinRace(id, alias, characterId)
    RPC.execute("rd-racing:joinRace", id, alias, characterId)
end
...
function leaveRace()
    ...
    if curRace then
        RPC.execute("rd-racing:dnfRace", curRace.id)
        cleanupRace()
    else
        RPC.execute("rd-racing:leaveRace")
    end
...
function getAllRaces()
    ...
    local res = RPC.execute("rd-racing:getAllRaces")
    races = res.races
...
function getHasRaceUsbAndAlias()
    ...
    local racingCreateUsbItem = exports["rd-inventory"]:GetInfoForFirstItemOfName("racingusb0")
    local racingUsbItem = exports["rd-inventory"]:GetInfoForFirstItemOfName("racingusb2")
    local pdRacingUsbItem = exports["rd-inventory"]:GetInfoForFirstItemOfName("racingusb3")
