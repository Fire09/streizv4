...
local function saveVehicle()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        TriggerEvent("rd-jobs:bennys:order", veh, changedStuff)
...
function AttemptPurchase(purchaseType, upgradeLevel)
    ...
    if bennys.access and (bennys.access == "police" or bennys.access == "ems") and purchaseType == "repair" then
        return STR.execute("rd-bennys:jobRepair")
    end
    ...
    if (bennys.access == "police" or bennys.access == "ems") then
        local free = exports['rd-config']:GetMiscConfig('bennys.pd.free')

        pd = free
    end
    TriggerServerEvent("rd-bennys:attemptPurchase", cheap, purchaseType, upgradeLevel, pd)
...
function RepairVehicle()
    Citizen.CreateThread(function ()
        ...
        if (missingEngineHealth > 50) then
            local finished = exports["rd-taskbar"]:taskBar(5000 + (missingEngineHealth / 50), "Repairing engine...", true, nil, nil, nil, nil, 6.0)
        ...
        if missingBodyHealth > 50 then
            local finished = exports["rd-taskbar"]:taskBar(5000 + (missingBodyHealth / 50), "Repairing body...", true, nil, nil, nil, nil, 6.0)
...
function PreviewMod(categoryID, modID)
    ...
    if categoryID == 39 or categoryID == 4 then
        if GetResourceState("rd-vehiclesync-state") == "started" then
            exports['rd-vehiclesync-state']:SetEngineSound(plyVeh, true)
        elseif GetResourceState("rd-vehiclesync") == "started" then
            exports['rd-vehiclesync']:SetEngineSound(plyVeh, true)
        end
    end
end
...
function ExitBennys()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        if GetResourceState("rd-vehiclesync-state") == "started" then
            exports['rd-vehiclesync-state']:SetEngineSound(plyVeh)
        elseif GetResourceState("rd-vehiclesync") == "started" then
            exports['rd-vehiclesync']:SetEngineSound(plyVeh)
        end
    ...
    TriggerServerEvent("rd-bennys:removeFromInUse", currentBennys)
    if devmode then
      TriggerEvent("vehicle:leftBennys", nil, nil)
    elseif not isAtPreviewBennys then
      exports["rd-ui"]:showInteraction("Bennys")
...
local function finishEnterLocation()
    ...
    isPlyInBennys = true
    TriggerServerEvent("rd-bennys:addToInUse", currentBennys)
    disableControls()
    TriggerEvent("inmenu", isPlyInBennys)
end
...
function enterLocation(pBennys, needsRepair)
    ...
    if isAtEmployedBennys then
        ...
        VehicleClass = exports["rd-vehicles"]:GetVehicleRatingClass(plyVeh)
    end
    ...
    isVinScratched = exports['rd-vehicles']:IsVinScratched(plyVeh)
...
--#[Event Handlers]#--
RegisterNetEvent("rd-bennys:purchaseSuccessful")
AddEventHandler("rd-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("rd-bennys:purchaseFailed")
AddEventHandler("rd-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    local isAllowed, message  = STR.execute("rd-bennys:isAllowedToUse", currentBennys, NetworkGetNetworkIdFromEntity(plyVeh))
    ...
    local isTestDrive = STR.execute("showroom:isTestDriveVehicle", NetworkGetNetworkIdFromEntity(plyVeh))
    ...
    if bennysAccess then
        ...
        jobAccess = exports["rd-business"]:IsEmployedAt(bennysAccess)

        if (isAtPreviewBennys and previewPermission[currentBennys] and isTestDrive) then
            jobAccess = exports["rd-business"]:HasPermission(bennysAccess, previewPermission[currentBennys])
        end
...
Citizen.CreateThread(function()
    Wait(5000)
    local catalog = STR.execute('rd-jobs:bennys:getPartsCatalog')

    local modifiers = STR.execute('rd-jobs:bennys:getPriceModifiers')
...
RegisterNetEvent('rd-admin:bennys', function()
    devmode = true
