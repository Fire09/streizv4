...
local function removeGuns()
  local qty = exports["rd-inventory"]:getQuantity("-2009644972")
  ...
  --local megaphoneQty = exports["rd-inventory"]:getQuantity("megaphone", true, { type = "paintball" })
  --if megaphoneQty and megaphoneQty > 0 then
    --TriggerEvent("inventory:removeItemByMetaKV", "megaphone", megaphoneQty, "type", "paintball")
  --end
  --local smokeQty = exports["rd-inventory"]:getQuantity("smokegrenadenpa")
  --if smokeQty and smokeQty > 0 then
    --TriggerEvent("inventory:removeItem", "smokegrenadenpa", smokeQty)
  --end
end

AddEventHandler("rd-paintball:getPaintballGun", function()
  TriggerEvent("player:receiveItem", "-2009644972", 1)
end)

AddEventHandler("rd-paintball:getCaddy", function()
  if not isNpa() then return end
  FYX.execute("rd:vehicles:basicSpawn", "caddy", { x = 5513.32, y = 5992.66, z = 590.01 }, 359.1, "paintball")
end)

AddEventHandler("rd-paintball:getPaintballSmoke", function()
  if not isNpa() then return end
  TriggerEvent("player:receiveItem", "smokegrenadenpa", 1)
end)

AddEventHandler("rd-paintball:getPaintballAmmo", function()
  TriggerEvent("player:receiveItem", "paintballs", 20)
end)

AddEventHandler("rd-paintball:getMegaphone", function()
  if not isNpa() then return end
...
RegisterNetEvent("rd-paintball:reviveAfterDown", function()
...
AddEventHandler("rd-paintball:setArenaType", function(pArgs)
  if not isNpa() then return end
  RPC.execute("rd-paintball:setArenaType", pArgs[1])
end)

RegisterNetEvent("rd-paintball:changeArenaType", function(pType)
...
Citizen.CreateThread(function()
  Citizen.Wait(60000)
  TriggerServerEvent("rd-paintball:getArenaType")
end)
...
AddEventHandler("rd-paintball:swapLocations", function()
  DoScreenFadeOut(400)
...
AddEventHandler("rd-paintball:enterArena", function()
  inArena = true
  TriggerEvent("rd-cleanup:enableCleanup", false)
  Citizen.CreateThread(function()
    while inArena do
      if currentLocation == "arena" then
...
AddEventHandler("rd-paintball:exitArena", function()
  inArena = false
  TriggerEvent("rd-cleanup:enableCleanup", true)
  removeGuns()
end)
