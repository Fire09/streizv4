let GlobalConfig = exports['rd-config'].GetModuleConfig('main');
...
on('rd-config:configLoaded', (pModule, pConfig) => {
    if (pModule === 'main') {
...
export function GetModuleConfig(pModule, pKey) {
    if (!TrackedModules.has(pModule)) {
        const config = exports['rd-config'].GetModuleConfig(pModule);
        if (config == undefined)
            return;
