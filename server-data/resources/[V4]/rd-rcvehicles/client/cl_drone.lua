...
RegisterNetEvent('rd-rcvehicles:drone:spawn', function(pDroneSettings)
...
RegisterNetEvent('rd-rcvehicles:drone:pilot', function(pDroneId)
  ...
  exports['rd-actionbar']:disableActionBar(true)
  ...
  TriggerEvent('rd-rc:droneEntered', CURRENT_DRONE)
  ...
  Citizen.CreateThread(function()
    ...
    while isPiloting do
      ...
      if IsDisabledControlJustPressed(0, Keys['SPACE']) then
        TriggerEvent('rd-rc:droneAbility', CURRENT_DRONE)
      end
    ...
    TriggerEvent('rd-rc:droneExited', CURRENT_DRONE)
    RPC.execute('rd-rcvehicles:leaveDrone', CURRENT_DRONE.id)
    if CURRENT_DRONE.crashed then
        RPC.execute('rd-rcvehicles:crashDrone', CURRENT_DRONE.id)
    end
    CURRENT_DRONE = nil
  end)
...
RegisterNetEvent('rd-rcvehicles:drone:destroy', function(pDroneId)
