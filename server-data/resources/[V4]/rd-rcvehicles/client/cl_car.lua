...
RegisterNetEvent('rd-rcvehicles:car:setup', function(pCarSettings)
...
RegisterNetEvent('rd-rcvehicles:car:spawn', function(pCarSettings)
...
RegisterNetEvent('rd-rcvehicles:car:pilot', function(pCarId)
  ...
  if not DoesEntityExist(CURRENT_CAR.handle) then
    DoScreenFadeIn(400)
    RPC.execute('rd-rcvehicles:leaveCar', CURRENT_CAR.id)
  ...
  TriggerEvent('rd-rc:carEntered', CURRENT_CAR)
  ...
  wexports['rd-actionbar']:disableActionBar(true)

  TaskWarpPedIntoVehicle(PlayerPedId(), CURRENT_CAR.handle, -1)
  TriggerEvent('rd-ui:setVehicleBypassed', CURRENT_CAR.handle, true)
  DoScreenFadeIn(1000)
  ...
  Citizen.CreateThread(function()
    ...
    exports['rd-actionbar']:disableActionBar(false)
    exports['rd-ui']:sendAppEvent('hud', { display = true })
    TriggerEvent('rd-ui:setVehicleBypassed', CURRENT_CAR.handle)

    SetNetworkIdCanMigrate(CURRENT_CAR.netId, true)

    TriggerEvent('rd-rc:carExited', CURRENT_CAR)
    RPC.execute('rd-rcvehicles:leaveCar', CURRENT_CAR.id)
    if CURRENT_CAR.crashed then
      RPC.execute('rd-rcvehicles:crashCar', CURRENT_CAR.id)
    end
    CURRENT_CAR = nil
  end)
...
RegisterNetEvent('rd-rcvehicles:car:destroy', function(pCarId)
