...
function Login.ShowMenu(toggle)
	...
	local lifersDisabled = exports["rd-config"]:GetMiscConfig("characters.lifers.disabled")
	local hardcoreDisabled = exports["rd-config"]:GetMiscConfig("characters.hardcore.disabled")
...
if data.action == "currentHover" then
	...
	local coords = exports["rd-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
...
if data.action == "deleteCharacter" then
	...
	local events = exports["rd-base"]:getModule("Events")
	events:Trigger("rd-base:deleteCharacter", data.actionData, function(deleted)
		Login.getCharacters(true)
	end)
end
...
if data.action == "newCharacter" then
	...
	if cData.type and cData.type == 'hardcore' then
		local canCreate = RPC.execute('rd-spawn:canCreateHardcoreCharacter')
		if not canCreate then callback({}) return end
	end

	if cData.type and cData.type == 'lifer' then
		local canCreate = RPC.execute('rd-spawn:canCreateLiferCharacter')
		if not canCreate then callback({}) return end
	end

	events:Trigger("rd-base:createCharacter", chardata, function(created)
		if not created then
...
RegisterNetEvent("rd-base:spawnInitialized")
AddEventHandler("rd-base:spawnInitialized", function()
	Login.nativeStart()
end)
