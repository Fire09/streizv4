...
AddEventHandler('spawn:clientSpawnData', function(spawnData, heheData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				-- TriggerEvent("rd-spawn:firstspawnyo")
			end
			return
		end
		...
		if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["rd-housing"]:retriveHousingTable()
		end
...
RegisterNetEvent("rd-spawn:firstspawnyo")
AddEventHandler("rd-spawn:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("rd-spawn:finishedClothingstuffbish")
    else
        TriggerEvent("rd-spawn:finishedClothingstuff")
    end
end)

RegisterNetEvent('rd-spawn:finishedClothingstuff')
AddEventHandler('rd-spawn:finishedClothingstuff', function()
	...
	Citizen.CreateThread(function()
        while true do
			...
			if IsCutsceneActive() then 
				...
				Wait(3000)
				FreezeEntityPosition(GetPlayerPed(-1), false)
				if not exports["rd-inventory"]:hasEnoughOfItem("idcard",1,false) then
					TriggerEvent("player:receiveItem","idcard",1,true)
				end
				if not exports["rd-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
...
RegisterNetEvent('rd-spawn:finishedClothingstuffbish')
AddEventHandler('rd-spawn:finishedClothingstuffbish', function()
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("rd-business:hasPermission", pBusinessCode, "key_access")
...
function Spawn.selectedSpawn(spawnInfo)
	...
	if apartment then
		...
		if pos then
			...
			DoScreenFadeIn(2500)
			TriggerEvent("rd-spawn:characterSpawned")
			-- TriggerEvent("rd-spawn:characterSpawned")
		else 
