...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("rd-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with rd-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("rd-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["rd-base"]:getModule("Events")
	events:Trigger("rd-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	        ...
	        TriggerEvent("rd-base:firstSpawn")
	    end
    end)
end
...
function Login.setClothingForChar()
    ...
    TriggerEvent("rd-clothes:client:CreateFirstCharacter")
...
RegisterNetEvent("rd-spawn:finishedClothing")
AddEventHandler("rd-spawn:finishedClothing", function(endType)
