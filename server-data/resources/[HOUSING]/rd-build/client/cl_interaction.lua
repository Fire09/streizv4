...
function Build.func.buildLine(interact)
	...
	if interact then
		...
		if interact.housingSecondary ~= nil and interact.housingSecondary[1] ~= "" then
			local key = exports['rd-keybinds']:getKeyMapping('+housingSecondary')
			local str1 = "[" .. key .. "]"
...
function Build.func.buildPeeking(plan)
	...
	exports["rd-interact"]:AddPeekEntryByModel({ peek.model }, {{
		event = peek.event,
		id = peek.id,
		icon = peek.icon,
		label = peek.label,
		...
		isEnabled = function(pEntity)
		  return exports["rd-housing"]:isInRobbery()
		end
	})
...
function Build.func.buildNPC(plan)
	...
	for k,v in pairs(npc) do
		...
		local seller = exports["rd-npcs"]:RegisterNPC(data, "rd-build")
		exports["rd-npcs"]:EnableNPC(seller.id)
...
function Build.func.removeNPC()
	...
	for k,v in pairs(Build.CurrentNPC) do
		exports["rd-npcs"]:DisableNPC(k)
		exports["rd-npcs"]:RemoveNPC(k)
	end
