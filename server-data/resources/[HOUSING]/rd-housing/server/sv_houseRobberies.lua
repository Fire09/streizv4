...
RegisterNetEvent("rd-base:sessionStarted", function()
    local src = source
    TriggerClientEvent("rd-housing:beingRobbed", src, Housing.housingBeingRobbed)
end)
...
STR.register("housing:robbery:pickedLock", function(src, pHouseId)
    ...
    TriggerClientEvent("rd-housing:beingRobbed", -1, pHouseId)
...
STR.register("getDestroyedTable", function(src, pHouseId)
...
STR.register("housing:robbery:robLocationsGenerated", function(src, pRobLocations, pHouseId)
...
STR.register("housing:robbery:searchedPoint", function(src, pHouseId, pSelected)
...
STR.register("property:alarmDisabled", function(src, pHouseId, pFail)
    ...
    for k, v in pairs(Housing.Players) do
        if v == pHouseId then
            ...
            if pFail then
                TriggerClientEvent("rd-housing:alarm", src, pHouseId)
            end
        end
    end
...
STR.register("housing:robbery:targetRemovePlayer", function(src, pHouseId)
