...
function generateZoneList()
    ...
    local shouldSend = STR.execute("housingShouldSetZoneLocations")
    if shouldSend then
        STR.execute("setZoneLocations", Housing.zone)
    end
end
...
function canRent()
    ...
    if isComplete and dist <= 3.0 then
        ...
        local canRent = STR.execute("rd-housing:canRent", propertyId)
...
AddEventHandler("property:menuAction", function(pData)
    ...
    if isComplete and dist <= 3.0 then
        ...
        if action == "lockdown" then
            STR.execute("property:clientLockdown",propertyID)
        elseif action == "checkOwner" then
            STR.execute("property:getOwner",propertyID)
...
AddEventHandler("rd-housing:handler", function(data, cb)
...
Citizen.CreateThread(function()
    exports["rd-keybinds"]:registerKeyMapping("","Housing", "Interact", "+propertyInteract", "-propertyInteract", "E")
