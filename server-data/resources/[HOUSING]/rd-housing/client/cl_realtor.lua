...
RegisterNetEvent("rd-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("rd-housing:getGarages", function()
    getIsPropLock()
end)
...
function getIsPropLock()
    ...
    -- print(_propertyID)
    TriggerServerEvent("rd-housing:isHousingLockedServer", _propertyID)
    local pp =  STR.execute("rd-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('rd-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = STR.execute("rd-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('rd-phone:editingHouse',id,street,price)
    else
        TriggerEvent('rd-phone:sentHouse',nil,id,street,price,category)
...
RegisterCommand("editmode", function(src, args, raw)
...
function enterEdit(pPropertyId)
    ...
    if playerInRangeOfProperty(propertyID) then
        ...
        return true, Housing.EditOptions
    else
        return false, "Too far from the property."
    end
end

function exitEdit(saveChanges)
    if saveChanges then
        local finished = STR.execute("updateCurrentSelected", Housing.currentlyEditing, Housing.currentHousingInteractions, Housing.hasEditedOrigin)
    end
...
function setGarage()
    ...
    if exports["rd-vehicles"]:IsOnParkingSpot(GetVehiclePedIsIn(PlayerPedId(), false), true, 5.0) then TriggerEvent("DoLongHudText","To close to another garage.",2) return false end
...
function setInventory()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        -- STR.execute("property:hasSwapped",Housing.currentHousingInteractions.id)
...
function placeBench(dropInventory)
    ...
    if dropInventory and hasBenches then
        ...
        STR.execute("property:dropInventory", vec, cat .. "-" .. propertyID, propertyID)
    end
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("rd-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["rd-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("rd-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("rd-housing:sellTo", function(pParams)
	...
	if input["cid"] then
		...
		TriggerServerEvent("rd-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("rd-housing:buy")
AddEventHandler("rd-housing:buy", function(pPropertyId, pSeller)
    local tax = Housing.info[pPropertyId]["price"]
    -- redothis
end)

RegisterCommand('rent', function()
    TriggerEvent('rd-housing:rent')
end)

RegisterNetEvent("rd-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("rd-housing:rent2", function(pParams)
    local complete, info = STR.execute("rd-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("rd-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = STR.execute("rd-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("rd-housing:edit", function()
    local context = {}
...
RegisterUICallback("rd-housing:editOptions", function(cb, data)
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["rd-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["rd-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["rd-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("rd-housing:blips")
AddEventHandler("rd-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = STR.execute("rd-housing:owned")
...
RegisterNetEvent('rd-housing:logShit')
AddEventHandler('rd-housing:logShit', function(hID, Price)
    TriggerServerEvent('rd-housing:buyLog', hID, Price)
end)