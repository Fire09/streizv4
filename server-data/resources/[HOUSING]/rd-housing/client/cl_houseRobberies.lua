...
function attemptToLockPickHouse(skipPicking)
    ...
    local finished, housingInformation, currentHousingLocks, isResult, housingLockdown, housingRobbed, robTargets = STR.execute("getCurrentSelected", propertyID)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["rd-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
            ...
            lockpicked, housingRobbed, robTargets, alarm = STR.execute("housing:robbery:pickedLock", propertyID)
...
function buildRobLocations(model, propertyID)
    ...
    local finished, locations = STR.execute("housing:robbery:robLocationsGenerated", robLocations, propertyID)
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["rd-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["rd-ui"]:showInteraction("[E] Take Object")
        else
            exports["rd-ui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["rd-inventory"]:getCurrentWeight()
    ...
    local finished, message = STR.execute("housing:robbery:searchedPoint", Housing.currentlyRobInside, Housing.currentClosestSelected)
    ...
    if Housing.alarm == true and math.random(1, 100) > 90 then
        ...
        STR.execute("property:alarmDisabled", Housing.attackedTarget, true)
    end
    ...
    if type(item) == "string" then
        ...
        local finished = exports["rd-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["rd-taskbar"]:taskBar(15000,"Searching Location")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            local complete,message = STR.execute("housing:robbery:destroyedObject", Housing.currentlyRobInside, model, vec3)
            if complete then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("rd-heists:complete", math.random(30, 50))
        ...
        local complete,destroyedTable = STR.execute("getDestroyedTable", Housing.currentlyRobInside)
...
RegisterNetEvent("rd-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("rd-housing:alarmFail")
AddEventHandler("rd-housing:alarmFail", function()
...
RegisterNetEvent("rd-housing:alarmEnter")
AddEventHandler("rd-housing:alarmEnter", function()
    insideGame = true
    if not Housing.alarm then return end
    for i=1,2 do
        if not failedAlarm then
            local finished = exports["rd-ui"]:taskBarSkill(3200,  math.random(10, 14))
            if finished ~= 100 then
                failedAlarm = true
            end
        end
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("rd-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("rd-housing:alarm")
AddEventHandler("rd-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["rd-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("rd-housing:beingRobbed", function(pPropertyId)
        ...
        if Housing.attackedTarget ~= nil then
            ...
            if propertyPos ~= nil and Housing.currentlyRobInside == false and not Housing.lockout then
                ...
                if dist > 100 then
                    ...
                    STR.execute("housing:robbery:targetRemovePlayer", Housing.attackedTarget)
                end
            end
