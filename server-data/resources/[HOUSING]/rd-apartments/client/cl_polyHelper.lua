...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports["rd-ui"]:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = _L("apartments-ui-apartment", "Apartment") .. " # "..k, action = "rd-ui:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
    ...
    if Apart.defaultInfo[1] == apartmentLocNumber and Apart.defaultInfo[2] == apartmentNumber then
        data[#data+1] = {
            ...
            action = "rd-ui:apartmentsContext"
        }
    end
  ...
  RegisterUICallback("rd-ui:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["rd-ui"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
