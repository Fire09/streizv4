  ...
  for _, laser in ipairs(LaserList) do
    laser.onPlayerHit(function(playerBeingHit, hitPos)
      if playerBeingHit then
        TriggerEvent("rd-heists:vault:laserHit", laser, hitPos)
        PlaySoundFromEntity(-1, "TIMER_STOP", PlayerPedId(), "HUD_MINI_GAME_SOUNDSET", 1, 5.0);
      end
    end)
  end
end)

RegisterNetEvent("rd-heists:vault:laserState", function(pState)
  for _, laser in ipairs(LaserList) do
