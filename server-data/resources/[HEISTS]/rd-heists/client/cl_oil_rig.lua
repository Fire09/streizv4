  ...
  exports["rd-interact"]:AddPeekEntryByFlag({ "isNPC" }, {
    {
        ...
        label = "Talk",
        icon = "comment",
        event = "rd-heists:OilRigNPCTalk",
        parameters = {},
    },
}, { distance = { radius = 2.5 }, npcIds = { "heist_dock_worker" } })
end)

AddEventHandler("rd-heists:OilRigNPCTalk", function()

  local NPCTalkMenu = {
    {
      title = "Start",
      action= "rd-heists:startOilRig",
    },
    {
      title = "Cancel",
      action= "rd-heists:cancelOilRig",
    }
  }
    exports["rd-ui"]:showContextMenu(NPCTalkMenu)

end)

RegisterUICallback('rd-heists:startOilRig', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' } })
  if exports["rd-inventory"]:hasEnoughOfItem("heistentrycard", 1) then
  SetNewWaypoint(-3555.163, 7359.035, 33.53)
  TriggerEvent("pd:deathcheck")
  DoScreenFadeOut(500)

  exports['rd-weathersync']:FreezeWeather(true, 'Foggy Rain')
  exports['rd-weathersync']:FreezeTime(1)
  TriggerEvent('reviveFunction')
  TriggerEvent('np-evidence:disable', true)
  ...
  exports['rd-police']:DisableBleeding(true)

  DoScreenFadeIn(10000)

  TriggerEvent("addEmail", {title = "Unknown", subject = "Special Mission", message = "I acquired some boats for you to use, they should be anchored nearby"})

  local data = {
    id = "boat_spawner",
    position = {
      coords = vector3(-2080.244, 2611.64, 2.08),
      heading = 109.96,
    },
    pedType = 4,
...
local boatSpawner = exports["rd-npcs"]:RegisterNPC(data, "rd-heists")

exports["rd-npcs"]:EnableNPC(boatSpawner)
...
exports["rd-interact"]:AddPeekEntryByFlag({ "isNPC" }, {
  {
      ...
      label = "Talk",
      icon = "comment",
      event = "rd-heists:spawnBoat",
      parameters = {},
  },
}, { distance = { radius = 2.5 }, npcIds = { "boat_spawner" } })
...
AddEventHandler("rd-heists:spawnBoat", function()
  spawnVehicle("patrolboat")
end)

RegisterUICallback('rd-heists:cancelOilRig', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' } })
end)

VEHICLE_SPAWN_LOCATIONS = {
  vector3(-2088.447, 2605.616, 0.516),
}
...
function spawnVehicle(pVehicle)
  ...
  local plate = "OIL ".. GetRandomIntInRange(1000, 9000)
