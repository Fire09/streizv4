...
local cashstackprice = 1800
local rollcashprice = math.random(200, 235)
local rollcount = 10
local bandcount = 10
...
local carspawns = {
[1] =  { ['x'] = -1328.4428710938,['y'] = -231.60423278809,['z'] = 42.206966400146,['h'] = 305.82315063477, ['info'] = ' car 8' },
[2] =  { ['x'] = -1330.1126708984,['y'] = -228.86077880859,['z'] = 42.282688140869,['h'] = 305.28076171875, ['info'] = ' car 1' },
[3] =  { ['x'] = -1331.9008789062,['y'] = -226.58770751953,['z'] = 42.352035522461,['h'] = 306.27407836914, ['info'] = ' car 2' },
}
-----------------------
------ NETEVENTS ------
-----------------------
...
if toolong < 0 then
	...
	local isNotInVeh = IsPedInAnyVehicle(ped, false)
	if dstcheck < 2.0 and pedCreated and isNotInVeh then
		local crds = GetEntityCoords(deliveryPed)
		exports['rd-ui']:showInteraction("[E] to drop off package")
		if IsControlJustReleased(0,38) then
			TaskTurnPedToFaceEntity(deliveryPed, PlayerPedId(), 1.0)
			Citizen.Wait(1500)
			PlayAmbientSpeech1(deliveryPed, "Generic_Hi", "Speech_Params_Force")
			local finished = exports["rd-taskbar"]:taskBar(4000,"Handing Over Package")
			local assluck = math.random(1,3)
			if assluck == 2 then 
				exports["rd-dispatch"]:dispatchadd("10-66", "Suspicious Activity")
			end
		...
		TriggerEvent("DoLongHudText", "Either you are in a vehicle or you are not close enough to the drop off.")
	end
end
...
TriggerEvent("addEmail", {title = "Anonymous", subject = "Oxy Run", message = "Locations will be forwarded to you soon. Don't get caught!"})
salecount = 0
firstdeal = true
OxyRun = true
vehspawn = false
...
RegisterNetEvent("rd-oxy:letsrun", function()
	if not OxyRun then 
		TriggerServerEvent("oxydelivery:server", 350)
	else
		TriggerEvent("DoLongHudText","Sorry can't help you right now.")
	end
end)
...
if drugdealer then
	...
	if salecount == 7 then
		TriggerEvent("addEmail", {title = "Anonymous", subject = "Oxy Run", message = "This is your last order, we are dry out of customers!"})
	...
	if (not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 100.0) and vehspawn then
		OxyRun = false
		tasking = false
		TriggerEvent("addEmail", {title = "Anonymous", subject = "Oxy Run", message = "Dude! You fucked the car up, I canceled your run asshole!"})
	else
		if tasking then
			Citizen.Wait(30000)
		else
...
Citizen.Wait(100)
TriggerEvent("vehicle:keys:addNew", oxyVehicle, plate)
...
if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
	...
	if math.random(10) == 1 then
		TriggerEvent( "player:receiveItem", "sledgehammer", 1 )
	end

	if OxyRun then
		local cashPayment = math.random(50,60)
		if exports["rd-inventory"]:hasEnoughOfItem("rollcash",rollcount) then
		...
		elseif exports["rd-inventory"]:hasEnoughOfItem("band",bandcount) then
		...
		elseif exports["rd-inventory"]:hasEnoughOfItem("cashstack",cashstackamount) then
			TriggerEvent("inventory:removeItem","cashstack", cashstackamount)
			cashPayment = cashPayment + cashstackprice
			TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
			TriggerServerEvent('mission:finished', "cashstack", cashPayment)
		else
