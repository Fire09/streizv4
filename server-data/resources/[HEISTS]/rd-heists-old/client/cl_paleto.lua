...
RegisterNetEvent('rd-paleto:box')
AddEventHandler('rd-paleto:box', function()
    ...
    if distance < 1.5 then
        if exports["rd-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
            ...
            PaletoStep1()
            exports["rd-dispatch"]:dispatchadd('10-90', "Paleto Bank Robbery", "108")
...
RegisterNetEvent('str:createpaletomoney')
AddEventHandler('str:createpaletomoney', function(toggle)
    if toggle == true then
...
function CreateMoney()
    ...
    TriggerEvent('str:CleanClientVaultD')
    TriggerEvent("str:createpaletomoney" ,true)
    Citizen.Trace("Crates Spawned :  Sucess")
    Citizen.Wait(240000)
    Citizen.Wait(60000)
    TriggerEvent("str:createpaletomoney" ,false)
end


RegisterNetEvent('rd-paleto:startpaleto')
AddEventHandler('rd-paleto:startpaleto', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(-102.1473, 6463.0869, 31.634153) - player)
    if distance < 1.5 then
            if exports["rd-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                StartPaleto()
                exports["rd-dispatch"]:dispatchadd('10-90', "Paleto Bank Robbery", "108")
                TriggerEvent("client:newStress",true, 10)
...
function OnHackSuccess()
    ...
    TriggerEvent("inventory:removeItem", "heistlaptop2", 1)
    TriggerServerEvent('rd-paleto:startCoolDown')
    loadDicts()
    ...
    TriggerServerEvent('rd-doors:change-lock-state', 45, false) 
    CreateMoney()
end

RegisterNetEvent('rd-paletogold')
AddEventHandler('rd-paletogold', function()
    print('called 2?')
    TriggerServerEvent('rd-paleto:tut_tut')
end)

AddEventHandler("rd-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('rd-paleto:box')
    elseif pItem == "heistlaptop2" then
        TriggerServerEvent("rd-paleto:hacklaptop")
    end
end)