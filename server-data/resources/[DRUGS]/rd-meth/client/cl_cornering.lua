...
AddEventHandler("rd-meth:cornerSellProduct", function(pArgs, pEntity)
    local gameTimer = GetGameTimer()

    if lastCornerSale and (gameTimer - lastCornerSale) <= 4000 then
        return TriggerEvent('rd-meth:cleanAttempt', lastCornerSale - gameTimer)
    end

    lastCornerSale = gameTimer

    if not exports["rd-inventory"]:hasEnoughOfItem("methlabproduct", 1, false, true) then
    ...
    local success = RPC.execute("rd-meth:attemptCornerSale", NetworkGetNetworkIdFromEntity(pEntity), inCornerId)
    if success then
        TriggerEvent("money:clean")
        TriggerServerEvent("rd-heists:generateSafeCrackingTool")
    end
...
AddEventHandler("rd-meth:cornerStartSelling", function()
    local result = RPC.execute("rd-meth:startCornering", inCornerId)
    if not result then return end
    TriggerEvent("rd-meth:showSellDrugsMenuItem", "cancorner", false)
    playerCornering = true
end)

AddEventHandler("rd-polyzone:enter", function(name, data)
    if name ~= "meth_corner" then return end
    if not exports["rd-inventory"]:hasEnoughOfItem("methlabproduct", 1, false) then return end
    inCorner = true
    inCornerId = 1
    -- inCornerId = data.id
    TriggerEvent("DoLongHudText", "Looks like a good spot to sell...")
    TriggerEvent("rd-meth:showSellDrugsMenuItem", "cancorner", true)
end)
AddEventHandler("rd-polyzone:exit", function(name, data)
    if name ~= "meth_corner" then return end
    if not inCorner then return end
    if playerCornering then
        RPC.execute("rd-meth:stopCornering")
...
RegisterNetEvent("rd-meth:findCandidateVehicle")
AddEventHandler("rd-meth:findCandidateVehicle", function(pCornerId, pSrc)
  PopulateNow()
  Wait(0)
  ...
  TriggerServerEvent("rd-meth:corneringFoundVehicle", pCornerId, pSrc, { vehicle = cornerVehicle, driver = cornerPed })
end)

RegisterNetEvent('rd-meth:findCandidate', function()
  PopulateNow()
  Wait(0)
  local vehicles = GetGamePool('CVehicle')
