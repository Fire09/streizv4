...
AddEventHandler('rd-inventory:closed', function(pName)
    ...
    if found then
        TriggerServerEvent('rd-gallery:fetchJewelry')
    end
    ...
    if found then
        TriggerServerEvent('rd-gallery:scrapJewelry', pName)
    end
end)

AddEventHandler('rd-gallery:jewelryAction', function(pParameters, pEntity, pContext)
    ...
    context[#context+1] = {
        title = 'Scrap Jewelry',
        icon = 'hammer',
        action = 'rd-gallery:jewelryMenuAction',
        key = { scrap = true }
    }
    --context[#context+1] = {
...
RegisterUICallback("rd-gallery:jewelryMenuAction", function(data, cb)
    ...
    if data.identifyAll then
        ...
        Citizen.CreateThread(function()
            exports['rd-taskbar']:taskBar(length, 'Identifying Jewelry')
        end)
        for _,item in ipairs(items) do
            local info = json.decode(item.information)
            if info.identified then
            ...
            NPX.Procedures.execute('rd-gallery:identifyJewelry', item)
            Wait(1000)
            ::continue::
        end
    end
    ...
    if data.identifyItem then
        ...
        local result = NPX.Procedures.execute('rd-gallery:identifyJewelry', item)
        if not result then
            return
        end
    ...
    if data.repairItem then
        ...
        local hasParts = exports['rd-inventory']:hasEnoughOfItem('jewelry_part', cost, false, true)
    ...
    if data.craft then
        ...
        print("striez test", type, cost)
        TriggerServerEvent('rd-gallery:craftJewelryItem', type, cost)
    end
end)
...
function GetJewelryInventoryItems(pType)
    ...
    if pType == 'jewelry' then
        local rings = exports['rd-inventory']:getItemsOfType('jewelry_ring', 10, false)
        local necklaces = exports['rd-inventory']:getItemsOfType('jewelry_necklace', 10, false)
    ...
    if pType == 'relics' then
        local relics = exports['rd-inventory']:getItemsOfType('jewelry_relic', 10, false)
...
RegisterNetEvent('rd-gallery:applyEffects', function(pEffects)
    appliedEffects = {}
    ...
    for _,effect in pairs(pEffects) do
        ...
        if effect.id == 'LUCKY_BONUS' then
            appliedLucky = true
            TriggerEvent("rd-buffs:applyBuff", "jewelry", { { buff = "jobluck", percent = (effect.level / 100), timeOverride = 12 * 60 * 60 * 1000 } })
        end
    end

    if not appliedLucky then
        TriggerEvent("rd-buffs:applyBuff", "jewelry", { { buff = "jobluck", percent = 0.0, timeOverride = 1 } })
    end
end)
...
AddEventHandler('rd-gallery:jewelry:armorRegen', function(pLevel)
...
AddEventHandler('rd-gallery:jewelry:healthRegen', function(pLevel)
...
AddEventHandler('rd-gallery:jewelry:stressReduce', function(pLevel)
    TriggerEvent("client:newStress", false, pLevel, true)
end)

AddEventHandler('rd-gallery:jewelry:jailTimeReduce', function(pLevel)
    if not isPrisoner then return end
    TriggerServerEvent('rd-jail:reducePrisonTime', pLevel)
end)

AddEventHandler("rd-jail:playerJailed", function(pMonths)
    if pMonths < 120 then
        isPrisoner = true
    end
end)
