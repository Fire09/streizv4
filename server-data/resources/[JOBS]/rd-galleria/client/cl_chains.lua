...
AddEventHandler("rd-inventory:itemUsed", function(pItem, pInfo)
  ...
  if not hasCgChainOn then
    ...
    TriggerEvent("rd-props:attachProp", model, 10706, -0.02, 0.02, -0.06, -366.0, 19.0, -163.0, true, true)
    Citizen.CreateThread(function()
      while hasCgChainOn do
        Citizen.Wait(math.random(10000, 120000))
        if info.diamond then
          local gemInfo = exports['rd-galleria']:getGemData(info.diamond)
          TriggerServerEvent("rd-fx:chain:blingDiamonds", GetEntityCoords(PlayerPedId()), "diamonds", gemInfo.count, 100)
        end
    ...
    Citizen.CreateThread(function()
      while hasCgChainOn do
        Citizen.Wait(math.random(10000, 120000))
        if info.ruby then
          local gemInfo = exports['rd-galleria']:getGemData(info.ruby)
          TriggerServerEvent("rd-fx:chain:blingDiamonds", GetEntityCoords(PlayerPedId()), "ruby", gemInfo.count, 100)
        end
      end
    end)
    Citizen.CreateThread(function()
      while hasCgChainOn do
        ...
        if info.tanzanite then
          local gemInfo = exports['rd-galleria']:getGemData(info.tanzanite)
          TriggerServerEvent("rd-fx:chain:blingDiamonds", GetEntityCoords(PlayerPedId()), "tanzanite", gemInfo.count, 100, 0.25)
        end
      end
    end)
  else
...
AddEventHandler('attachedItems:block', function(status)
    ...
    elseif (hadattachitem) and not status then
        hadattachitem = false
        TriggerEvent("rd-inventory:itemUsed",storedpItem, storedpInfo)
    end
end)
...
AddEventHandler("rd-inventory:itemUsed", function(pItem, pInfo)
  ...
  if not hasSquidMaskOn then
    hasSquidMaskOn = true
    -- SetPedComponentVariation(PlayerPedId(), 7, chainValue, 0, 0)
    TriggerEvent("rd-props:attachProp", model.model, 31086, model.x, model.y, model.z, -13.0, -86.0, -165.0, true, true)
  else
    hasSquidMaskOn = false
    TriggerEvent("rd-props:removeProp")
  end
...
RegisterNetEvent("rd-inventory:itemCheck", function(item)
  local model = chainModels[item]
  if not model then return end
  local qty = exports["rd-inventory"]:hasEnoughOfItem(item, 1)
  if qty then return end
  hasCgChainOn = false
  TriggerEvent("rd-props:removeProp")
end)
local function hasChainAccess()
  local hasAccess = exports['rd-business']:HasPermission('jeweled_dragon', 'craft_access')
...
AddEventHandler("rd-clothing:openCGChainCrafting", function()
  if not hasChainAccess() then return end
  TriggerEvent("server-inventory-open", "42087", "Craft")
end)
AddEventHandler("rd-clothing:infuseChainWithGems", function()
  if not hasChainAccess() then return end
  local cid = exports["isPed"]:isPed("cid")
  NPX.Procedures.execute("rd-gallery:infuseCGChain", cid)
end)

-- Citizen.CreateThread(function()
--   TriggerEvent("rd-props:attachProp", "np_squid_vip", 31086, 0.03, 0.03, 0.0, -13.0, -86.0, -165.0, true, true)
-- end)
