...
RegisterServerEvent('rd-jail:pJailedLog')
AddEventHandler('rd-jail:pJailedLog', function(months, jailed_player)
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    local user2 = exports["rd-base"]:getModule("Player"):GetUser(jailed_player)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** riddle [Jail] | Log **",
          ["description"] = "Officer Name: "..charInfo.first_name.. ' '..charInfo.last_name .."\n Months: "..months.."\n Jailed Player: "..charInfo2.first_name.. ' '..charInfo2.last_name,
        }
      }
      PerformHttpRequest("https://discord.com/api/webhooks/1012082729698988063/Yd3XLReL_0vOnrVm0TSM-hIVg4ZHbXeyuTwNY_7M9rSmMVF0w1wuHow9rh56lNhWfY_o", function(err, text, headers) end, 'POST', json.encode({username = "riddle | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)

RegisterServerEvent('rd-jail:pBilledLog')
AddEventHandler('rd-jail:pBilledLog', function(pFineAmount, fined_player)
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    local user2 = exports["rd-base"]:getModule("Player"):GetUser(fined_player)
    local charInfo = user:getCurrentCharacter()
    local charInfo2 = user2:getCurrentCharacter()
    ...
    local connect = {
      ...
      PerformHttpRequest("https://discord.com/api/webhooks/1012082458130383039/saKe3jMeCWz_Mwbh13U2oHywvlbf43E8uwHznsTyF75SYICNcvuAOX2ZGH4J-lR-Pd7p", function(err, text, headers) end, 'POST', json.encode({username = "riddle | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)

RegisterServerEvent("server-jail-item")
AddEventHandler("server-jail-item", function()
    local src = source
    local user = exports["rd-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
