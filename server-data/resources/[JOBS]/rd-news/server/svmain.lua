	...
	if Config.Whitelist then
	    if isAllowedToChange(usource) then
			if article then
				...
				MySQL.Async.execute("UPDATE newspaper SET reporter=@reporter, articleTitle1=@articleTitle1", {["@reporter"] = GetPlayerIdentifiers(usource)[1], ["@articleTitle1"] = table.concat(args, " ")})
			else
	...
	if Config.Whitelist then
	    if isAllowedToChange(usource) then
			if article then
				...
				MySQL.Async.execute("UPDATE newspaper SET reporter=@reporter, articleTitle2=@articleTitle2", {["@reporter"] = GetPlayerIdentifiers(usource)[1], ["@articleTitle2"] = table.concat(args, " ")})
			else
				TriggerClientEvent('chatMessage', usource, '', {255,255,255}, '^8Error: ^1You did not put in a title.')
			end
		else
	...
	if Config.Whitelist then
	    if isAllowedToChange(usource) then
			if article then
				...
				MySQL.Async.execute("UPDATE newspaper SET reporter=@reporter, articleDesc1=@articleDesc1", {["@reporter"] = GetPlayerIdentifiers(usource)[1], ["@articleDesc1"] = table.concat(args, " ")})
			else
				TriggerClientEvent('chatMessage', usource, '', {255,255,255}, '^8Error: ^1You did not put in a title.')
			end
		else
	    ...
	    if isAllowedToChange(usource) then
			if article then
				TriggerClientEvent("newspaper:headline2", -1, article)
	 			MySQL.Async.execute("UPDATE newspaper SET reporter=@reporter, articleDesc2=@articleDesc2",{["@reporter"] = GetPlayerIdentifiers(usource)[1], ["@articleDesc2"] = table.concat(args, " ")})
			else
				TriggerClientEvent('chatMessage', usource, '', {255,255,255}, '^8Error: ^1You did not put in a title.')
			end
		else
