...
Citizen.CreateThread(function()
    ...
    local areCountersPublic = exports['rd-config']:GetMiscConfig("business.counters.public")
    for _, counter in ipairs(activeCounters) do
        if counter.public and not areCountersPublic then goto continue end
        exports["rd-polytarget"]:AddBoxZone(counter.name, counter.polytarget[1], counter.polytarget[2], counter.polytarget[3], counter.polytarget[4])
        exports["rd-interact"]:AddPeekEntryByPolyTarget(counter.name,
            {
                {
