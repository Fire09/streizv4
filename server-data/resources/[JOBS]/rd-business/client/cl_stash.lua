...
RegisterCommand("creationmenu", function()
	...
	local hasPerm = RPC.execute("rd-business:hasPermission", "ace_industries", "craft_access")
	
	if employed and hasPerm then
		TriggerEvent("rd-business:stash:creationMenu")
	end
end)
...
RegisterUICallback("rd-business:stash:enteredAuthCode", function(data, cb)
...
function getPDAuthCode()
  ...
  exports["rd-ui"]:openApplication("textbox", {
    callbackUrl = "rd-business:stash:enteredAuthCode",
    key = "",
    items = {
...
function openInventory(type, data)
  ...
  if type == "stash" then
    local success = RPC.execute("rd-business:hasPermission", biz, "stash_access")
  ...
  elseif type == "craft" then
    local success = RPC.execute("rd-business:hasPermission", biz, "craft_access")
    if not success and isPD() then
      success = getPDAuthCode()
    end
    if success then
...
function enterPoly(name, data)
  ...
  exports["rd-ui"]:showInteraction("[E] " .. name)
end
...
local creationMenu = {
  key = 1,
  show = true,
  callbackUrl = "rd-business:client:createStash",
  items = { }
}

Citizen.CreateThread(function()
  exports["rd-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      id = "business_stash",
      event = "rd-business:client:openStash",
      icon = "archive",
      label = "Open Storage",
    },
  }, 
  ...
  exports["rd-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      ...
      event = "rd-business:client:removeStash",
      icon = "trash",
      label = "Remove Storage",
    },
    {
      id = "business_stash_update",
      event = "rd-business:client:changeStashModel",
      icon = "archive",
...
RegisterNetEvent("rd-business:stash:creationMenu")
AddEventHandler("rd-business:stash:creationMenu", function ()
...
AddEventHandler("rd-business:client:removeStash", function (pData, pEntity)
  local objInfo = exports["rd-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end

	TriggerEvent("animation:PlayAnimation", "hammering")

  local finished = exports["rd-taskbar"]:taskBar(15000, "Destroying Storage")
  ClearpedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local deleted = exports["rd-objects"]:DelObject(objInfo.id)
  if not deleted then
...
AddEventHandler("rd-business:client:openStash", function (pData, pEntity)
  local objInfo = exports["rd-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local success = RPC.execute("rd-business:hasPermission", businessCode, "stash_access")
  if not success and isPD() then
    success = getPDAuthCode()
  end
...
AddEventHandler("rd-business:client:changeStashModel", function (pData, pEntity)
  local objInfo = exports["rd-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local menuData = {
    key = { id = objInfo.id },
    show = true,
    callbackUrl = "rd-business:client:updateStashModel",
    items = {
      { icon = "object-ungroup", _type = "select", label = "New Unit Model", name = "stashProp", options = unitOptions },
    }
  }
  exports["rd-ui"]:openApplication("textbox", menuData)
end)

RegisterUICallback("rd-business:client:updateStashModel", function(pData, pCb) 
  ...
  local finished = exports["rd-taskbar"]:taskBar(15000, "Changing Storage")
  ClearpedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local updated = exports["rd-objects"]:UpdateObject(pData.key.id, stashProp)
end)

RegisterUICallback("rd-business:client:createStash", function(pData, pCb) 
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  exports["rd-ui"]:closeApplication("textbox")
