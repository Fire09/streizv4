...
FoodItems = CacheableMap(function (ctx, pRestaurant)
  local items = STR.execute("rd-restaurants:getFoodItems", pRestaurant)
...
AvailableMainDishes = CacheableMap(function (ctx, pRestaurant)
  local items = STR.execute("rd-restaurants:fetchMenu", pRestaurant)
  if not items then return false, nil end
...
AddEventHandler("rd-restaurants:createFoodItem", function(p1, p2, pContext)
  ...
  for _, item in pairs(foodItems) do
    ...
    contextMenu[#contextMenu + 1] = {
      ...
      icon = "utensils",
      key = item,
      action = "rd-ui:restaurant:cookFoodItem",
  ...
  exports["rd-ui"]:showContextMenu(contextMenu)
end)

RegisterUICallback("rd-ui:restaurant:cookFoodItem", function(data, cb)
  ...
  while true do
    if math.random() < 0.05 then
      local finished = exports["rd-ui"]:taskBarSkill(math.random(2000, 8000), math.random(7, 12))
    ...
    for _, item in pairs(items) do
      for _, ingred in pairs(uniqueIngredients) do
        if hasCategory(item.foodCategory, ingred) then
          ...
          local qty = exports["rd-inventory"]:getQuantity(item.itemName, true)
    ...
    local success = exports["rd-taskbar"]:taskBar(5000, "Preparing...", true, false, false, false, nil, 0.5, PlayerPedId())
    if success ~= 100 then break end
    local metaDatas = {}
    for ingred, items in pairs(currentIngredientCounts) do
      local processedCount = 0
      for itemName, count in pairs(items) do
        local itemData = exports["rd-inventory"]:getItemsOfType(itemName, count, true)
...
RegisterNetEvent("rd-restaurants:changedFoodItems", function(pRestaurant)
  resetFoodItems(pRestaurant)
end)

RegisterNetEvent("rd-restaurants:changedMenuItems", function(pRestaurant)
